...
Five containers on one host, built there, published to the internet through
the Cloudflare tunnel that already runs on that machine.
...
| Name | Purpose |
| --- | --- |
| `APP_BASE_URL` | Public origin. Baked into the frontend image because `robots.txt` and `sitemap.xml` are generated at build time. |
...
`DEPLOY_PORT`. The host key is pinned rather than accepted on first use:
