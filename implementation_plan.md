# Woldeok Moneyverse 통합 개발·운영·배포 파이프라인 구현 계획서 (현재: v72)

## 📜 누적 버전 히스토리 (Version Changelog & Diffs)
- **v72**: 전 도메인 미구현 백엔드 API 전수 완성(마켓 잉글리시 옥션·P2P 1:1 직거래·공인 감정소, 클럽 12x12 캔버스 동기화, 컬렉션 큐레이션 리텐션 허브) 및 프론트엔드 실시간 통신 완전 연동 착수 (+180, -0)
...
## 🚀 [v70 Specification] P2P 플레이어 마켓플레이스 & 에스크로 경매장 최종 확정 사양 및 액션 플랜 (누적 추가)

### 1. 조율 확정 사양 (Final Confirmed Decisions via ask_question)
1. **[에스크로 실시간 경매(English Auction)]**:
   - 공개 호가 방식: 현재 최고 입찰가(`highest_bid`) 실시간 갱신.
   - 상위 입찰 발생 시 직전 입찰자의 WLD는 에스크로 락에서 즉시 100% 원자적 환불(`ESCROW_REFUND`).
   - 스나이핑 방지(Anti-Sniping): 경매 마감 30초 이내 신규 입찰 발생 시 마감 시간이 60초 자동 연장.
   - 낙찰 시 출품자에게 대금 정산 및 경매 수수료 2% 영구 소각(`SINK_AUCTION_FEE`).
2. **[P2P 1:1 안전 직거래(Direct P2P Escrow)]**:
   - 양방향 2단계 확인 서명 (Two-Way Dual Sign-Off):
     - 발신자: 상대방 유저명 지정 + 교환 희망 물품 및 제안 WLD 제시 (`PROPOSED`).
     - 수신자: 실시간 제안 내용 검토 후 수락 (`ACCEPTED`).
     - 최종 확인: 양측 모두 '최종 교환 승인' 서명 시 시스템 에스크로를 통해 원자적 동시 스왑 (`SWAPPED`). 사기·어뷰징 원천 차단.
3. **[공인 시스템 감정소(Appraisal Service) 및 희귀도 인증]**:
   - 기획서 §5.3 준수: 감정 수수료 `max(250 WLD, ceil(reference_value * 0.0025))` 영구 소각 (`SINK_APPRAISAL_FEE`).
   - 공인 감정 완료 시 고유 시리얼 번호, 원작자 출처(Provenance), 최근 20건 P25~P75 시세 밴드 보증 배지 발급.
4. **[시세 및 가격발견(Price Discovery) 대시보드]**:
   - 토스/로빈후드형 롤링 중앙값(Rolling Median) & 7일/30일 거래량/체결가 SVG 스파크라인.
   - 비정상 고가/저가 탐지 시 '시세 대비 과도' 경고 태그 부착.
5. **[작업 진행 방식]**:
   - 자율 실행 모드: 컴포넌트 개발, 테스트, Turbopack 프로덕션 빌드, 미니 PC 무중단 블루-그린 승격(v394)까지 AI가 원스톱 완결.

### 2. 세부 컴포넌트 개발 및 변경 파일
1. `frontend/src/app/marketplace/auction-view.tsx` [NEW]:
   - 잉글리시 옥션 실시간 목록, 남은 시간 카운트다운 타이머, 호가 입찰 모달, 상위 입찰 환불 안내 및 스나이핑 연장 뱃지.
2. `frontend/src/app/marketplace/direct-trade-view.tsx` [NEW]:
   - 1:1 직거래 제안 생성 모달, 상대방 인벤토리/제안 WLD 실시간 미리보기, 2단계 상호 확인 서명 인터페이스.
3. `frontend/src/app/marketplace/appraisal-view.tsx` [NEW]:
   - 공인 감정소 메인 뷰, 감정 의뢰(수수료 실시간 계산 및 소각), 공인 인증서 카드 발급 및 시세 밴드 뷰어.
4. `frontend/src/app/marketplace/price-discovery-chart.tsx` [NEW]:
   - 7일/30일 체결가 및 거래량 SVG 스파크라인, 롤링 중앙값 메트릭, 이상치 경고 칩.
5. `frontend/src/app/marketplace/marketplace-tabs.tsx` [MODIFY]:
   - [고정가 마켓 | 실시간 경매장 | 1:1 P2P 직거래 | 공인 감정소 | 제작 작업대] 5대 탭 통합 마운트.

### 3. 검증 및 프로모션 계획
- 프론트엔드/백엔드 유닛 테스트 100% PASS 확인.
...
## 🚀 [v72 Specification] 전 도메인 미구현 백엔드 API 전수 완성 및 프론트엔드 실연동 사양 (누적 추가)

### 1. 개요 및 배경 (PROJECT_PLAN v397 준수)
- **배경**:
  - `PROJECT_PLAN.ko.md` v2026.09.23.397 P0 불변조건: 모든 구현 기능은 서버 상태, 영속화, 권한판단 비즈니스 로직에 대응하는 API를 필수 구현해야 함.
  - 마켓플레이스 경매/1:1직거래/감정소, 클럽 12x12 캔버스, 컬렉션 큐레이션 리텐션 허브의 백엔드 API가 누락되어 프론트엔드 모의 데이터로 방치된 갭 전면 해소.
  - v395에서 신설된 세무 구청 및 시즌 정산 API의 프론트엔드 화면 연동 완결.

### 2. 도메인별 신규 백엔드 API 명세
1. **P2P 마켓플레이스 경매 & 직거래 & 감정소 (`backend/src/marketplace/`)**:
   - `GET /api/v1/marketplace/auctions`: 실시간 잉글리시 옥션 목록 (최고 입찰가, 입찰 수, 남은 시간, 스나이핑 연장).
   - `POST /api/v1/marketplace/auctions`: 신규 경매 등록 (물품 에스크로 락, 시작가, 즉시구매가).
   - `POST /api/v1/marketplace/auctions/:id/bid`: 호가 입찰 (직전 입찰자 100% 즉시 에스크로 자동 환불, 마감 30초 내 입찰 시 60초 자동 연장).
   - `GET /api/v1/marketplace/trades`: 내 1:1 직거래 제안 목록 (`PROPOSED`, `ACCEPTED_BY_PEER`, `COMPLETED`, `CANCELLED`).
   - `POST /api/v1/marketplace/trades`: 1:1 직거래 제안 발신 (제안 물품/WLD 및 요청 물품/WLD 에스크로 락).
   - `POST /api/v1/marketplace/trades/:id/accept`: 1차 수락 (`ACCEPTED_BY_PEER`).
   - `POST /api/v1/marketplace/trades/:id/confirm`: 양방향 2단계 최종 승인 서명 및 원자적 동시 스왑 (`COMPLETED`).
   - `POST /api/v1/marketplace/trades/:id/cancel`: 제안 취소 및 에스크로 반환.
   - `GET /api/v1/marketplace/appraisals`: 발급된 공인 인증서 목록 (시세 밴드 P25~P75, 고유 시리얼 해시).
   - `POST /api/v1/marketplace/appraisals`: 공인 감정 의뢰 (`max(250 WLD, ceil(0.25%))` 100% 영구 소각 `SINK_APPRAISAL_FEE`).
2. **클럽하우스 12x12 공유 캔버스 (`backend/src/club/`)**:
   - `GET /api/v1/clubs/:id/canvas`: 12x12 가구 배치 그리드 및 장식 점수 조회.
   - `PUT /api/v1/clubs/:id/canvas`: 12x12 가구 배치 저장 (운영진 권한 검증 및 실시간 동기화).
3. **컬렉션 소유권 & 큐레이션 리텐션 허브 (`backend/src/collection/`)**:
   - `GET /api/v1/collections`: 내 수집품 조각 목록 조회 (유래, 희귀도, 획득일).
   - `PUT /api/v1/collections/:id`: 수집품 애착 메모(`userNote`) 및 즐겨찾기(`isFavorite`) 토글.
   - `GET /api/v1/collections/curation/status`: D1~D7 큐레이션 리텐션 사다리 진행 상태 조회.
   - `POST /api/v1/collections/curation/advance`: D1~D7 다음 단계 개방 및 카드 생성.

### 3. 프론트엔드 실연동 계획 ("그다음 거 마저 해줘")
- `frontend/src/app/marketplace/auction-view.tsx`: 실제 백엔드 경매 API 연동.
- `frontend/src/app/marketplace/direct-trade-view.tsx`: 실제 백엔드 1:1 직거래 API 연동.
- `frontend/src/app/marketplace/appraisal-view.tsx`: 실제 백엔드 공인 감정소 API 연동.
- `frontend/src/app/clubs/[clubId]/clubhouse-canvas.tsx`: 실제 백엔드 캔버스 저장/조회 API 연동.
- `frontend/src/app/collections/curation-retention-flow.tsx`: 실제 백엔드 컬렉션 API 연동.
- `frontend/src/app/spaces/`: v395 신규 세무 구청 부동산세 납부 및 체납 공매 확인 UI 연동.
- `frontend/src/app/seasons/`: v395 신규 시즌 현황, 명예의 전당 트로피 쇼케이스 및 보상 수령 UI 연동.

### 4. 검증 및 프로모션 계획
- DB 마이그레이션 232번 적용.
- 백엔드/프론트엔드 단위 테스트 100% 통과.
- 프로덕션 컴파일 빌드.
- 미니 PC 무중단 블루-그린 승격 (v396) 및 1,069+ 세션 무손실 보존.
