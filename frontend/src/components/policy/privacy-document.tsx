...
export function PrivacyDocument() {
  return (
      ...
      <tr><th scope="row">프로필·갤러리 사진</th><td>이용자가 선택한 이미지 파일, 저장 키, 설명문, 공개 범위, 제출·검토 상태 및 연결된 내부 사용자 ID</td><td>프로필 표시, 갤러리 제출·운영 검토, 권한에 따른 이미지 제공, 악성·권리침해 콘텐츠 대응</td><td>프로필 사진은 교체·삭제 요청 시 접근을 차단하고 파일 삭제 절차를 진행합니다. 갤러리 제출물의 최종 보유 기간과 탈퇴 시 처리 기준은 운영 전 확정·게시가 필요합니다.</td></tr>
      </tbody>
      </table>
      </div>
      ...
      <p>사진 파일 및 갤러리 제출 기록은 위 표의 확정 전 항목입니다. 운영 전에는 탈퇴·삭제 요청 시 파일, 메타데이터, 공개 게시물의 처리 순서와 보유 기간을 확정해 이 방침에 반영해야 합니다.</p>
      </section>
