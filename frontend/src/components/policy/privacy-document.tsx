...
export function PrivacyDocument() {
  return (
      ...
      <tr><th scope="row">보안·감사</th><td>내부 사용자 ID, 요청 ID, 작업 종류·결과·시각, 권한 변경·관리 작업 기록. 관리자 요청에 한하여 접속 IP 주소, User-Agent에서 파생한 브라우저·운영체제 구분과 되돌릴 수 없는 해시, 세션 식별자의 단방향 해시, HTTP 메서드·경로, 추적 ID, 응답 상태와 처리시간</td><td>보안사고 대응, 권한 통제, 부정 이용 조사</td><td>인증·권한 거부 기록 90일, 관리자·경제 감사 기록 1년</td></tr>
      ...
      <p>위 표의 접속 IP 주소와 브라우저·세션 관련 값은 운영 콘솔에 접속한 관리자의 요청에만 기록하며, 일반 이용자의 요청에는 기록하지 않습니다. 관리자 화면에서도 IP 주소는 네트워크 단위로, 세션 식별자는 해시 일부만 보이며, 원본을 확인하면 누가 왜 확인했는지가 사유와 함께 다시 기록됩니다.</p>
      </section>
      <section>
