...
const rows = [
  { id: '1', symbol: 'AAA', name: 'Alpha', current_price: '1000000000000000000000000000001', day_open_price: '1000000000000000000000000000000', shares_available: '2' },
  { id: '2', symbol: 'BBB', name: 'Beta', current_price: '300', day_open_price: '200', shares_available: '9000000000000000000000000000000' },
  { id: '3', symbol: 'CCC', name: 'Gamma', current_price: '90', day_open_price: '100', shares_available: '5' },
];
...
describe('stock market sorting', () => {
  ...
  it('sorts integer-string values without Number precision loss', () => {
    ...
    expect(sortMarketStocks(rows, 'available').map((row) => row.symbol)).toEqual(['BBB', 'CCC', 'AAA']);
  });

  it('sorts percentage movers by exact cross multiplication', () => {
    expect(sortMarketStocks(rows, 'change').map((row) => row.symbol)).toEqual(['BBB', 'AAA', 'CCC']);
  });
