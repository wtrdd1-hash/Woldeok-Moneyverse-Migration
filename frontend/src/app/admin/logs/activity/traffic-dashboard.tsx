...
export function TrafficDashboard({ data }: { readonly data: TrafficAnalyticsDashboard }) {
  ...
  return <div className="grid gap-4">
    ...
    <Card><CardHeader><CardTitle className="text-base">{labels[data.granularity]}별 접속 추이</CardTitle><CardDescription>한국 시간 기준 집계입니다.</CardDescription></CardHeader><CardContent className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left text-muted-foreground"><th className="py-2">기간</th><th>페이지뷰</th><th>세션</th><th>로그인 사용자</th></tr></thead><tbody>{data.series.map((row) => <tr key={row.bucket} className="border-b"><td className="py-2 font-mono text-xs">{row.bucket}</td><td>{nf.format(row.pageViews)}</td><td>{nf.format(row.uniqueSessions)}</td><td>{nf.format(row.authenticatedUsers)}</td></tr>)}</tbody></table></CardContent></Card>
