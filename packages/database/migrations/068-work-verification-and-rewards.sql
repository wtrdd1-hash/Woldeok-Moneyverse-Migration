-- Verifying a submitted task and paying for it.
--
-- The reward is minted: MINT is credited and the member's cash account is
-- debited, which in this schema is the direction that moves value to the
-- member (init/001 line 37 adds a debit and subtracts a credit).
--
-- Two caps bound the mint, a day and a week, and they are read and written
-- under row locks taken in the same order every time. The reward is clamped
-- to whatever is left of both, so exhausting a cap pays zero rather than
-- failing -- the work is still recorded and the experience is still awarded.
...
BEGIN
  ...
  -- The member, not the account. `admin_set_user_restriction` (026) sets
  -- `users.status` and does not freeze accounts or revoke sessions, so a
  -- member restricted for economy abuse still holds a live session and a
  -- live cash account. `economy_claim_work` (021) checked this; losing the
  -- check while replacing that path would have been a regression.
  IF NOT EXISTS (
