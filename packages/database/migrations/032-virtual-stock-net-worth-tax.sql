-- Game tax policy: beginning players are never taxed.  A sell is taxed only
-- when the user's total game net worth (cash + bank + marked-to-market stock
-- positions) exceeds 1,000,000 WLD.  The tax is 1% of that sale and is capped
-- at the amount above the threshold, so a player cannot be taxed below it.
...
DECLARE
  ...
  v_trade uuid;
  v_next bigint;
  v_wallet_net_worth bigint := 0;
  v_stock_net_worth bigint := 0;
  v_net_worth_before_tax bigint := 0;
BEGIN
  ...
  -- Idempotency must return the original receipt without changing a position,
  -- balance, price, or outbox event a second time.
  RETURN QUERY
  SELECT trade.id, trade.unit_price, trade.gross_amount, trade.tax_amount, stock.current_price
  FROM public.virtual_stock_trades AS trade
  JOIN public.virtual_stocks AS stock ON stock.id = trade.stock_id
  WHERE trade.idempotency_key = p_key;
  IF FOUND THEN RETURN; END IF;

  -- Lock existing positions and their prices in a deterministic order before
  -- computing the portfolio value used by the tax rule.
  ...
  SELECT current_price, day_open_price
  INTO v_price, v_open
  FROM public.virtual_stocks
  WHERE id = p_stock AND active
  FOR UPDATE;
  ...
  WHERE owner_user_id = p_actor
    AND account_type = 'USER_CASH'::public.account_type
    AND status = 'active'::public.account_status
  FOR UPDATE;
  SELECT id INTO v_sink
  FROM public.accounts
  WHERE system_key = 'sink'
    AND account_type = 'SINK'::public.account_type
    AND status = 'active'::public.account_status
  FOR UPDATE;
  SELECT id INTO v_treasury
  FROM public.accounts
  WHERE system_key = 'treasury'
    AND account_type = 'TREASURY'::public.account_type
    AND status = 'active'::public.account_status
  FOR UPDATE;
  ...
  SELECT quantity INTO v_position
  ...
  IF p_side = 'sell' THEN
    SELECT coalesce(sum(balance.available_amount), 0)
    INTO v_wallet_net_worth
    ...
    SELECT coalesce(sum(position_row.quantity * stock_row.current_price), 0)
    INTO v_stock_net_worth
    FROM public.virtual_stock_positions AS position_row
  ...
  PERFORM public.economy_post_transaction(
    p_key,
    'VIRTUAL_STOCK_' || upper(p_side),
    p_actor,
    NULL,
    CASE
      WHEN p_side = 'buy' THEN jsonb_build_array(
  ...
  IF p_side = 'buy' THEN
    ...
    SET average_cost = ((virtual_stock_positions.quantity * virtual_stock_positions.average_cost + excluded.quantity * excluded.average_cost) / (virtual_stock_positions.quantity + excluded.quantity)),
        quantity = virtual_stock_positions.quantity + excluded.quantity,
        updated_at = now();
  ELSE
  ...
  v_next := CASE
    WHEN p_side = 'buy' THEN least((v_price * 101 + 99) / 100, (v_open * 115) / 100)
    ELSE greatest((v_price * 99) / 100, (v_open * 85) / 100, 10)
  END;
