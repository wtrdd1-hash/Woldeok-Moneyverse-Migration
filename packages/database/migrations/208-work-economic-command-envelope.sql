...
-- Update version: v2026.09.19.247
...
-- Preserve every work invariant accumulated by earlier migrations (feature state,
-- server clock, per-task/global quota, locking, receipts and progression).  The
-- public entry point becomes a thin command-envelope wrapper instead of copying
-- and accidentally regressing that policy logic.
...
BEGIN
  ...
  -- Execute the latest pre-envelope policy in this same transaction. Any policy,
  -- ledger, receipt or progression failure rolls the command claim back as well.
