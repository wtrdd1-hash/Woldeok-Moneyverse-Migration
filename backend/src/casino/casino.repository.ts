...
export class CasinoRepository {
  ...
  async clock(): Promise<ServerGameClockRow> {
    const row = await queryOne<ServerGameClockRow>(
      this.pool,
      `SELECT clock.policy_version,
              clock.day_index::text AS day_index,
              clock.week_index::text AS week_index,
              clock.day_of_week,
              clock.real_seconds_per_day,
              clock.game_days_per_week,
              clock.day_started_at,
              clock.day_ends_at,
              clock.week_started_at,
              clock.week_ends_at
