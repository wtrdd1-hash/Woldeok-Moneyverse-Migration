 ...
 *
 * Opening one rotates the authenticated administrator session — the
...
export class AdminSecurityController {
  ...
  @Get()
  @ApiOperation({ summary: 'Console session state and login policy' })
  async overview(@Req() request: RequestWithSession) {
    const session = requireSession(request);
    const actor = requireUserId(request);
    const service = this.service();
    const [policy, consoleSession] = await Promise.all([
