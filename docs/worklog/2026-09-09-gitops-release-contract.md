...
The current production host is Kubernetes/containerd reconciled by Flux, but the application repository still contained a Docker Compose/SSH production rollout workflow and release documentation describing the retired topology. Since the previous audit, `kuber-infrastructure` also added a complete isolated `wdmv-test` stack, making the old "no test server" documentation stale.
...
- `.github/workflows/deploy.yml`
  ...
  - kept advertising fail-closed by default;
  - scoped package-write permission to the image-build job;
  - explicitly reports that production promotion is a reviewed GitOps change.
- `docs/architecture/deployment-flow.md`
  - documents CI -> isolated test -> production image -> GitOps PR -> Flux -> Kubernetes rollout -> smoke/data audit.
- `docs/operations/production-deployment.md`
  - replaces the stale Compose/container procedure with Flux/GitOps rollout and rollback rules.
- `docs/RELEASING.md`
  - replaces the stale Compose procedure with the authoritative English GitOps release guide.
- `docs/RELEASING.ko.md`
  - adds the Korean translation.
- `docs/INDEX.md`
  - links both release guides.
...
- 광고 기본값 fail-closed 유지
- package write 권한을 이미지 build job으로 제한
- GitOps PR만 운영 변경 정본으로 명시
- 격리 테스트 환경을 정식 사전 배포 게이트로 문서화
- 배포 아키텍처·운영 배포 문서·릴리스 문서를 Kubernetes/Flux 기준으로 교체
- 영문 공식 릴리스 문서와 한국어 번역본 분리 및 문서 인덱스 연결
...
- GitHub Actions의 외부 action full-SHA pinning은 아직 남아 있어 #129는 완전히 해결되지 않았습니다.
- main required CI 강제는 #127에서 계속 추적해야 합니다.
- 관리자 2단계 인증의 frontend/backend 계약 불일치 등 애플리케이션 보안 잔여 이슈는 이번 release-engineering 변경 범위 밖이며 별도 수정이 필요합니다.

### 검증 상태

이 작업은 최종 PR HEAD에서 secret scan, lint, typecheck, build, PostgreSQL migration, tests, Prisma guard, production dependency audit를 모두 통과한 뒤에만 main에 병합합니다.
