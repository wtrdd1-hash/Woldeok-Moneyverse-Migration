...
## v2026.09.22.347 추가 확장: 신문 허브(Newspaper) 및 대화형 개발자 포털(/developer)

| 기능 | 앱 API 루트 / 대표 경로 | 지원 범위 |
| --- | --- | --- |
| **신문 허브** | `/app-api/v1/newspaper/*` | 시장 심리 지수(`pulse`), 주간 전망 투표(`poll`), 투표 참여(`poll/vote`), 금융 교육 아티클(`lore`) |
| **개발자 포털** | `/developer` (웹 포털) | 전 도메인 163개 API 실시간 카탈로그, 3개 언어 스니펫, 실시간 샌드박스 테스터 |


---

## v2026.09.22.358 추가 확장: 4대 신규 도메인 API 16종 및 전체 335개 엔드포인트 완비

| 기능 도메인 | 앱 API 루트 / 대표 경로 | 지원 범위 및 엔드포인트 |
| --- | --- | --- |
| **저축 포켓** | `/app-api/v1/banking/pockets` | 포켓 목록, 생성, 메인 계좌 ↔ 포켓 간 입출금 이체, 수정, 해지 환원 (5종) |
| **제작 워크벤치** | `/app-api/v1/crafting/*` | 4대 레시피 조회(`recipes`), 재료 소모 제작 실행(`execute`) (2종) |
| **유저 마켓플레이스** | `/app-api/v1/marketplace/*` | 활성 매물 검색(`listings`), 내 출품(`my-listings`), 출품 등록, 즉시 구매, 출품 취소 (5종) |
| **인앱 알림 센터** | `/app-api/v1/notifications/*` | 알림 피드, 미확인 카운트(`unread-count`), 개별 읽음, 전체 일괄 읽음(`read-all`) (4종) |
