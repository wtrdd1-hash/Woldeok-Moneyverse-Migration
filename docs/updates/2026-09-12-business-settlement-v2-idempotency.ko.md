...
`business_settle_daily_v2`가 저장소의 멱등성 표준을 따르지 않고 있었습니다.
...
3. 구형 V1 함수는 migration 085에서 이미 위 두 보호를 적용하고 있어 V2가 회귀한 상태였습니다.
...
6. 이후 기존 사업체 잠금, 일일 정산 제한, 부스트 계산, 원장 기록, 정산 영수증 생성 수행

기존 부스트 계산식, 원장 분개, 이벤트 이름과 payload는 변경하지 않았습니다.

## 테스트 추가

`backend/src/business/business-settlement-v2.db.test.ts`를 추가했습니다.

- 설치된 함수 정의에서 advisory lock이 재생 조회보다 먼저 위치하는지 확인
