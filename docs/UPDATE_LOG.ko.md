## v2026.09.26.458 — Phase 3 스타 드롭 5연속 탭 승급·프레스티지 환생 시스템·4인 공동 저축 팟 및 SEO 검색 유입 가이드 풀스택 배포, 단위 테스트 100% PASS, 1,433개 세션 무손실 무중단 운영 승격

- **적용 릴리스**: `prod-v458` (Git 커밋: `3d7c302c0fa21e25e9e0fa9506fc000aa275c9fc`)
- **운영 인프라**: Debian 미니PC 블루-그린 무중단 롤링 배포 (`stage_v458.sh`, `promote_v458.sh`)
- **PostgreSQL 활성 세션**: **1,433개 세션 100% 무손실 보존 완료**
- **주요 기능 업데이트**:
  1. **Brawl Stars형 스타 드롭 5연속 탭 승급 연출 & 투명 확률 공개 (`StarDropModal`)**:
     - 상자 탭 시 60% 확률로 희귀 ➡️ 초희귀 ➡️ 에픽 ➡️ 신화 ➡️ 전설 단계별 승급.
     - 2024년 개정 게임산업진흥법 제33조 준수 실시간 공정 확률표 모달 탑재.
     - 일일 3회 무료 탭 제공 및 VIP 확정 승급 럭키 참(Lucky Charm) 토글.
  2. **Cookie Clicker형 프레스티지 (환생) 시스템 & 대규모 통화량 소각 (`/progression/prestige`)**:
     - 누적 자산 100% 명예 소각(Reset)의 대가로 영구 패시브 생산력 배수(x2~x50) 적용.
     - 3대 고대 유물(황금 시계, 미다스의 손, 헤르메스의 인장) 슬롯 장착.
     - VIP 자산 보존 보험 티켓 사용 시 자산의 30%를 다음 생으로 이월.
  3. **Toss형 4인 소셜 공동 저축 챌린지 팟 & 친구 찌르기 (`/bank/savings-pot`)**:
     - 친구 4인이 모여 7일간 완주 시 +5% 보너스 금리 및 황금 상자 잭팟 분배.
     - 중도 포기/탈퇴 시 10% 페널티 풀 원천 소각. 100% 익명 닉네임 친구 찌르기(Nudge).
  4. **SEO 검색 최적화 & 법적 컴플라이언스 SSOT 공개 가이드 (`/guide/dopamine-system`)**:
     - 네이버/구글 검색 유입용 Article Schema JSON-LD, 메타데이터, 법적 고지 뱃지 완비.
- **품질 및 검증**:
  - 프론트엔드 Vitest 전체 단위 테스트 **134개 파일 / 857개 테스트 전수 100% 통과 (0 failed)**.
  - Next.js Turbopack 최적화 프로덕션 빌드 성공.
  - 전 서버 헬스체크 200 OK (`/`, `/guide/dopamine-system`, `/prediction`, `/marketplace/auction`, `/stocks`).

## v2026.09.26.457 — 10대 글로벌 레퍼런스 기반 Phase 2 도파민 & 지속 가능한 수익화(Monetization BM) 아키텍처 풀스택 배포, 단위 테스트 100% PASS 및 1,433개 활성 세션 무손실 무중단 운영 승격

- **적용 릴리스**: `prod-v457` (Git 커밋: `5ac988fe2bb113f8c87158913926ea6984e1b4db`)
- **운영 인프라**: Debian 미니PC 블루-그린 무중단 롤링 배포 (`stage_v457.sh`, `promote_v457.sh`)
- **PostgreSQL 활성 세션**: **1,433개 세션 100% 무손실 보존 완료**
- **주요 기능 업데이트**:
  1. **Duolingo형 7일 스트릭 내기 & 10인 주간 승강 리그 (`StreakWagerModal`)**:
     - 7일 연속 출석 및 일일 미션 달성 시 배팅금의 200% 즉시 지급 (리텐션 락인).
     - 스트릭 프리즈(연속 방어권) 계정당 3회 무료 제공 후 상점 유료 구매 연결.
     - 10인 주간 리그(브론즈~다이아몬드) 상위 3명 승급 / 하위 3명 강등.
  2. **Twitch형 실시간 호가창 슈퍼챗 & 골드 폭죽 롤링 티커 (`OrderbookSuperchat`)**:
     - 주식 거래소 호가창 상단 및 홈 대시보드에 실시간 골드 슈퍼챗 방송.
     - 금액별 4대 티어(브론즈/실버/골드/다이아) 및 Canvas 골드 폭죽 축하 애니메이션.
  3. **Steam형 P2P 아티팩트 경매장 & 5% 수수료 자동 소각 (`/marketplace/auction`)**:
     - 유저 간 한정판 호가창 테마, 움직이는 뱃지, 칭호, 프리즈 팩 P2P 경매 거래.
     - 경매 낙찰 시 플랫폼 거래 대금 5% 즉시 영구 소각으로 가상 화폐 인플레이션 방지.
- **품질 및 검증**:
  - 프론트엔드 Vitest 전체 단위 테스트 **131개 파일 / 846개 테스트 전수 100% 통과 (0 failed)**.
  - Next.js Turbopack 최적화 프로덕션 빌드 성공.
  - 전 서버 헬스체크 200 OK (`/`, `/prediction`, `/marketplace/auction`, `/stocks` 등).

## v2026.09.25.444 — 수익화·수익 근거 재검토 및 분산 수익성 계약

- OpenAlex/Crossref 후보 35,429건을 수집하고 DOI 우선/제목 보조 방식으로 중복 제거해 **고유 탐색 후보 33,341건**을 구축했다.
- 광고, 구독/프리미엄, 디지털 상품 직접판매, 플랫폼/마켓 수수료, 스폰서/크리에이터 모델, 모바일 IAP, 가격정책, 반복매출을 다시 검토했다.
- 현재 포트폴리오를 무료 핵심 + 맥락형 공개광고 + Moneyverse Plus/광고제거 편의 + 비P2W 코스메틱 직접판매 + 명확한 스폰서십으로 채택하고, 후기 creator/B2B/API는 별도 게이트를 둔다.
- 유료 WLD, 유료 확률형 아이템, 유료 카지노 가치, P2W 경제력, 유료 우월 WDX 정보는 계속 차단한다.
- 수익성은 스토어/결제/세금/환불/지원/모더레이션/콘텐츠/컴플라이언스 비용 차감 후 기여이익으로 측정하고 유지율·신뢰·안전 실험 가드레일을 둔다.
- `PROJECT_PLAN`, `INTEGRATED_PLANNING_MASTER`, `MONETIZATION_COMPLIANCE_SEO_SPEC`, 리서치 리뷰/코퍼스, 델타, 작업기록, GitHub용 업데이트 노트를 영/한 동기화했다.
- 문서/리서치 전용이며 runtime, Test, Production, 실제 매출 또는 법적 승인 완료를 주장하지 않는다.
...
- **P0/P1 핵심 보안 권한 격상(Step-Up 2FA) 및 경제 멱등성 보장 통합**:
  1. **경제 및 자산 변경 돌연변이 멱등성 보장 (Idempotency Contract)**:
     - 클럽(PR #684), 공간/스페이스(PR #682), 자산 재정의(PR #681), 주식 운영자(PR #680) 핵심 돌연변이 호출 시 멱등성 키(`idempotencyKey`) 필수 검증 계약 및 중복 실행 방지 확립.
  2. **핵심 관리자 및 위험 기능 Step-Up 2FA 가드 전면 적용**:
     - 상점 카탈로그(PR #672), 감사 로그(PR #671), AI 뉴스(PR #670), 시장 이벤트(PR #669), 상점 경제(PR #668), 관리자 역할(PR #667), 직업 정책(PR #666), 안전 센터 긴급 삭제(PR #661), 주식 관리(PR #659), 공개 콘텐츠(PR #657)에 최근 재인증 및 CSRF 가드 결합.
  3. **인프라 게이트 및 기획 동기화**:
     - 모바일 앱 API 최소 버전 통제 게이트(PR #678), 격리 테스트 회원가입 언블락(PR #683), 백엔드 필수 CI 린트 강화(PR #679, #673, #665), 국고 및 1:1 채팅 기획 동기화(PR #674, #646, #645, #644).
  4. **단위/E2E 테스트 및 빌드 검증**:
     - 백엔드 Vitest 97개 파일, 974개 테스트 100% 통과 (0 failed).
     - 프론트엔드 알림 테스트(3/3 PASS) 및 Next.js Turbopack 90여 개 라우트 최적화 빌드 완료.
  5. **무중단 운영 승격 (Zero-Downtime Blue-Green Promotion)**:
     - 테스트 서버(`https://test.easy-scraping.com/`) HTTP 200 OK.
     - 운영 서버(`https://easy-scraping.com/`) HTTP 200 OK, 알림 BFF 200 OK, 쪽지 미확인 401 Unauthorized 검증.
     - Nginx 에러 0건, **PostgreSQL 활성 사용자 세션 1,061건 100% 무손실 보존 실측 완료**.

## v2026.09.22.368 — 시간별 권위 기획 재정합

- v352↔v360 권위 드리프트, 폐기된 관리자 TOTP의 현재 통제 표기, mobile/API 계약 혼재 버전, 미병합 shop reauth 후보, release 증거 범위를 G368-01~05로 기록했다.
- 영/한 PROJECT_PLAN·changelog·worklog를 동기화했다. 기획/문서 전용이며 새 구현·배포 완료를 주장하지 않는다.
