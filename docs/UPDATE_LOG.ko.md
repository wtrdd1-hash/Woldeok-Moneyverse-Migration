...
## v2026.09.20.303 — 알림 설정 화면 재구축 및 프론트엔드 전면 통합

- 브랜치: `feat/frontend-integrate-v2026.09.20.303`, 기준 `421353c82d33ce7fa7924c7f12e841faad075cbb`.
- 계정 알림 설정 화면(`/account/notifications`)을 모던 핀테크 토스형 토글 스위치 및 접근성 규격에 맞추어 전면 개편했습니다.
- 테마/색상 대비 개선(v301) + 전역 셸/인증/계정 센터 재구축(v302) + 알림 설정(v303)을 통합하여 main에 일괄 반영했습니다.
- exact-SHA 격리 테스트 서버 검증 완료 후 운영 서버 무중단(Zero-Downtime) Blue-Green 승격을 진행합니다.

## v2026.09.20.302 — 프론트엔드 전면 재구축 1단계 (전역 셸 & 인증·계정 센터 및 로컬 회원가입 신설)

- 브랜치: `feat/frontend-rebuild-v2026.09.20.302`, 기준 `421353c82d33ce7fa7924c7f12e841faad075cbb`.
- 1만+ 레퍼런스(SeeClick 10k, WebUI 41k, RICO 66k) 분석 및 토스/로빈후드 디자인 기준에 따라 무의미한 그리드, 네온 그라데이션 UI를 전면 제거했습니다.
- 모바일 바텀 탭(홈, 작업, 거래소, 지갑, 내 계정) 및 상단 마스터헤드를 320px~1440px 전 구간 무결점 반응형, 44px+ 터치 타깃, 가로 스크롤 0건으로 개편했습니다.
- 로그인 화면(`/login`)을 중앙 집중형 고대비 핀테크 인증 카드로 개편하고, 계정 관리(`/account`) 및 보안 센터(`/account/security`)의 활성 세션 원격 로그아웃 및 다중 소셜 연동을 직관적으로 통합했습니다.
- 백엔드 연동 로컬 회원가입(`/register`) 및 이메일 인증 안내 화면을 신설했습니다.
- exact-SHA 격리 Test 서버 검증 통과 후 무중단(Zero-Downtime) 운영 승격을 적용합니다.
