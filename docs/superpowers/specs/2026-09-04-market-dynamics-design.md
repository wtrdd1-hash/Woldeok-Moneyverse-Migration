...
체결가 = next                                             -- 주문을 낸 쪽이 충격을 치른다(슬리피지)
fair += (next - price) * 0.3                             -- 30 %는 영구적으로 적정가에 남는다
```

- **체결은 충격 반영 후 가격이다.** 053처럼 호가에 체결하고 나서 가격을 올리면,
  유통량 3 %를 100에 사서 105가 된 가격에 되팔아 4 %를 챙기는 왕복이 무한히
  가능하다(세금은 1 %). 매수는 자기가 올린 가격에, 매도는 자기가 내린 가격에
  체결되므로 왕복은 언제나 손해다.
