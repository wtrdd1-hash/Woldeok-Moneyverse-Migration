...
## A5 — 자동 경제 조정 엔진 (089–092)

기획서 §15를 구현한다. §15는 "과거 원장을 수정하지 않고 **다음 정책 버전을 생성**하는
방식으로만" 동작하라고 못박는다. 그 기구는 059가 이미 만들어 뒀다 — `economy_policies`의
버전 체계, `economy_activate_due_policies`, `admin_rollback_economy_policy`. A5는 그 위에
얹는다. **새 싱글턴 테이블을 만들지 않는다.**

### 가장 큰 판단 — 엔진은 게임이 읽는 숫자를 움직여야 한다

다른 세션이 보내온 초안은 `economy_auto_policy` 테이블에 상한·감쇠율을 따로 두고 그것을
조정했다. 문제는 `work_verify_and_reward`(068)가 그 테이블을 읽지 않는다는 것이다. 엔진이
계산하고 감사 로그를 남기고 경보까지 울리지만 **회원이 겪는 것은 아무것도 바뀌지 않는다.**
그래서 089의 knob 레지스트리는 조정 대상마다 *실제로 값이 들어가는 컬럼*을 지정하고,
092의 적용기가 거기에 쓴다.
...
| 판단 | 이유 |
|---|---|
| knob마다 **기준값 컬럼**(`baseline_price` 등)을 따로 둔다 | 퍼센트를 지난주 결과에 곱하면 복리다. 주당 +5%를 열한 번이면 +70%인데, 매주 §15.4를 지키면서 그렇게 된다. 항상 기준값에서 계산한다 |
| 적용은 **트리거**, 활성화 함수의 한 단계가 아니다 | 버전을 active로 만드는 경로가 둘(스윕·롤백)이다. 스윕 안에 넣으면 롤백은 행만 되돌리고 가격표는 지난주 값으로 남는다 |
| knob 표에 **카지노·대출 항목이 없다** | §15.4의 두 제외를 구조로 표현한 것. 산술로 도달할 수 없다 |
| 거래 수수료·소비세 knob을 **만들지 않았다** | 스키마에 없다 — 꺼져 있는 게 아니라 부재한다. 새로 물리는 것은 요금 설계 결정이지 조정 엔진이 곁다리로 도입할 것이 아니다. §15.3의 해당 두 규칙은 존재하는 knob(상점가·사업 유지비)으로 대응한다 |
| 같은 knob에 대한 규칙 기여를 **합산 후 1회 clamp** | 물가를 올리라는 규칙과 내리라는 규칙이 같은 주에 참일 수 있다. 목록 마지막 규칙이 이기는 대신 상쇄된다. 상쇄된 규칙도 제안서에 남는다 |
| 제안기는 **읽기 전용 함수** | §15.4가 요구하는 "판단 근거 기록"이 실제 근거이려면, 계산해서 저장한 그 객체를 그대로 적용해야 한다. 스위치가 꺼진 서버에서 "오늘이면 뭘 할 건가"를 물을 수도 있다 |
| 주간 작업에 **행위자가 없다** | 059가 활성화 스윕에서 내린 결론과 같다. 아무도 수행하지 않은 산술에 책임질 사람을 발명하지 않는다. 흔적은 정책 행 자체다 |
| 대사 실패 차단은 **새로 짜지 않았다** | 088이 이미 실패 시 `economy_auto_policy`를 safe_mode로 내린다. 제안기는 스위치가 `enabled`가 아니면 거부한다. 창 안의 실패도 따로 본다 — 스위치를 다시 켜도 그 주의 숫자가 믿을 만해지지는 않으므로 |
| 표본 부족 판정을 **날짜별로 저장** | 하루가 비어 있는 주는 평균이 틀린 주다. 제안 시점에 한 번 판단하면 그걸 볼 수 없다 |
| 예금 이율은 **가감(delta) knob** | 041이 유통량에 따라 2/3/4bp를 낸다. 평평한 값으로 대체하면 반응을 knob과 맞바꾸는 것이다. ±2bp를 얹고 적립이 받는 1–10으로 clamp한다 |
| 공급·가격 규칙은 **품목별이 아니라 상점 전체** | 다이얼이 하나다. 한 품목의 품절은 사람이 보라고 경보로 올린다 |

### 마이그레이션

| 번호 | 내용 |
|---|---|
| 089 | knob 레지스트리, 기준값 컬럼과 그것을 채우는 트리거, `economy_policies`의 §15.4 기록 컬럼, 예금 이율 delta |
| 090 | §15.1 지표의 일일 스냅샷 (KST 하루 1행) |
| 091 | 창 요약과 §15.3 규칙의 제안기, §15.4 한계의 clamp |
| 092 | 적용기와 활성화 트리거, 주간 실행, 최고관리자 명령 |

`scheduler.ts`에 두 작업이 붙는다 — 매일 04:00 지표 수집, **월요일 04:30** 자동 조정.
지표가 먼저 도는 것이 순서상 필수다.
