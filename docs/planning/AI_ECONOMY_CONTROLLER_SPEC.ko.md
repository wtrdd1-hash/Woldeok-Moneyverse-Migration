...
## 36. 다음 작업 필수 AI 전수 재감사·재구축 게이트 — v2026.09.20.292

**다음 AI 개발 작업은 신규 AI 기능 개발보다 먼저 지금까지 만든 모든 AI 관련 구현을 전수 재감사하는 단계로 시작해야 한다.** 최소 범위는 경제 AI 검토/제어, 전문 에이전트 council/runtime, 직업·일일 제한 자동조절, 주식/뉴스 시나리오 생성·게시, 관리자 AI 상태/제어, scheduler/job, 저장·감사 기록, AI 상태를 노출하는 프론트/모바일 화면, 배포 설정, 모델/runtime health check, 그리고 AI 출력을 허용·거부하는 모든 결정론적 가드레일이다.

이 작업은 문서만 다시 읽는 검토가 아니다. 각 AI 기획 요구사항을 현재 소스 구현과 실제 런타임 동작까지 추적해야 한다. 최초 구현 이후 요구사항이 수정됐거나, 일부만 구현됐거나, 호환 패치가 겹겹이 붙었거나, 중복·stale·충돌 상태인 부분은 **기존 코드가 있다는 이유로 유지하지 말고 최신 요구사항을 기준으로 다시 설계하고 필요한 범위를 재구축한다.**

다음 작업의 강제 순서는 다음과 같다.

1. **AI 전체 표면 인벤토리:** 모든 AI 기획 항목, 코드 경로, migration/schema 객체, scheduler/job, 환경/config 키, 관리자/사용자 UI, 테스트, 운영 문서, 배포 증거를 나열하고 기획됨 / 구현됨 / 런타임 입증됨 / 부분구현 / stale / 충돌 / 제거·재구축 상태를 기록한다.
2. **코드 수정 전 최신 권한 재확인:** 최신 PROJECT_PLAN.md, 본 명세, 연결된 AI/경제/주식/직업 명세, 최신 사용자 지시, 현재 main을 다시 읽는다. 더 최신 지시와 충돌하는 과거 GitHub 기획은 초안으로 취급한다.
3. **계약·동작 감사:** AI 입력/출력 schema, 결정론적 validator, 권한, 감사 가능성, idempotency, scheduler 동작, 모델/runtime health, fail-closed, rollback, Test/Production 설정 분리, 그리고 설정됨 / 모델 호출됨 / AI 검토됨 / 적용 자격 있음 / 실제 적용·게시됨 상태 구분이 정확한지 확인한다.
4. **수정·취약 영역 재구축:** 요구가 바뀌었거나 patch-on-patch가 된 AI 컴포넌트는 최신 계약 기준으로 모듈/흐름을 다시 만든다. 안전한 migration이 가능하면 obsolete 분기, 중복 adapter, 오해를 만드는 fallback을 제거한다. 데이터/이력은 최신 schema·감사 계약상 필요한 것만 보존한다.
5. **시스템 전체 회귀검증:** 프론트엔드, 백엔드, DB, scheduler, API, 인증/session, 관리자, 모바일/반응형, 결정론적 경제/시장 권한을 함께 검증한다. feature switch가 켜졌거나 모델 endpoint가 정상이라는 이유만으로 AI가 정상 작동한다고 판정하면 안 된다.
6. **Test 런타임 입증:** 정확한 candidate SHA를 Test 환경에 배포하고 백엔드/API/DB/scheduler/AI runtime을 실제 Test 증거로 확인하며 성공·fail-closed·rollback 경로를 모두 실행하고 정확한 버전/config hash를 기록한다. 필수 AI 경로가 설정만 됨, 미입증 상태면 운영 승격 금지다.
7. **운영 승격 직전 재확인:** Production 승격 직전에 최신 기획과 사용자 지시를 다시 읽는다. 감사/재구축 중 요구가 바뀌었으면 구현을 최신화하고 영향 테스트를 다시 통과해야 한다.
8. **무중단 Production 승격:** 모든 게이트를 통과한 경우에만 기존 무중단 배포 경로로 승격하고 로그인 상태를 유지하며 승격 후 공개/백엔드 health를 확인하고 rollback 증거를 남긴다.

필수 산출물은 AI 구현 매트릭스, 변경범위/재구축 목록, 제거·유지한 호환 계층 목록, Test 증거, 실패/rollback 증거, 정확한 source SHA/config hash, 내부용·GitHub용 업데이트 내역이다. 이 전수 재감사·재구축 게이트를 완료하거나 남은 차단 항목을 명시적으로 범위 제외 승인하기 전에는 **새로운 AI 작업을 시작할 수 없다.**

### 36.1 버전 기록

- **v2026.09.20.292-01** — AI 전체 표면 인벤토리 및 최신 기획 재확인.
- **v2026.09.20.292-02** — AI/결정론 validator/scheduler/data/UI/deployment 구현·런타임 계약 감사.
- **v2026.09.20.292-03** — 변경·stale·부분구현·패치 누적 AI 영역을 최신 요구사항 기준으로 재구축.
- **v2026.09.20.292-04** — 정확한 candidate SHA로 Test 전체 회귀, fail-closed, rollback 검증.
- **v2026.09.20.292-05** — 최종 기획 재확인 후 모든 증거 통과 시에만 무중단 Production 승격 및 Production smoke 검증.
