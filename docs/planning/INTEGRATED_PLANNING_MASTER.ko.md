...
- 작업 중 main 재확인 결과 `4812a99b0a78da736e477d0a3a2f02ed4ea66283` 유지, 이번 회차 전 기획 브랜치 head는 `48b582e4e7914c476c9f57b769efd77cbc6de868`.
- 기존 무제한 플레이 원칙을 가상경제 인플레이션 레퍼런스와 현재 즉시정산 위험 기준으로 재검토했다.
- G399-01 / P0 경제설계: 즉시 WLD 정산이 무제한이면 클릭 빈도가 화폐발행을 지배해, 뒤에서 일일 cap을 걸어도 그 전에 지갑이 빠르게 팽창할 수 있다.
- 즉시 faucet 의미를 서버 권위 시간 기반 작업, 일반 WLD 지급 슬롯 1개, 제한된 WLD/시간 발행 밴드, 반복감소, 반복 hard sink, 경제건전성 피드백 제어로 대체했다.
- Test 시작값만 1시간 300–600 WLD, 고급작업 최대 1.5배, 저난도 반복 25–40% 하한으로 두고 Production 수치는 실제 지갑/sink 데이터의 7/30/90일 시뮬레이션 후 결정한다.
- faucet/sink 비율, 화폐공급 변화, 유통속도, currency-on-hand, 시장 바스켓 가격, 자산집중 지표를 필수화했다.
- 기획/문서 전용이며 런타임 완료를 주장하지 않는다. 영/한 동기화 완료.
