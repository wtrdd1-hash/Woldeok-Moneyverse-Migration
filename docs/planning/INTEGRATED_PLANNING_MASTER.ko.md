...
- 시작/중간 `origin/main=99b0eaa04bbd0b28005861c624690c56744e8a14`; 기록된 중간 점검 시 main drift 없음.
- 변경 채택 전 문서 권위, 현행 수익화/결제/국제화/성장 기획과 최신 update/work 기록을 다시 읽었다.
- 추적 가능한 수익화 탐색 코퍼스 구축: OpenAlex/Crossref 수집 35,429건 -> DOI 우선/제목 보조 중복 제거 후 **고유 후보 33,341건**. 33,341건 전수 원문 수작업 검토로 표현하지 않는다.
- **M444-01 / P1:** 분산 포트폴리오 = 무료 핵심 + 맥락형 공개광고 + 광고제거/편의/표현 구독 + 비P2W 코스메틱 직접판매 + 명확한 스폰서십. 후기 creator/B2B/API는 별도 게이트를 둔다.
- **M444-02 / P1:** 결제 전환을 강요하려고 무료 핵심을 의도적으로 악화하지 않으며 프리미엄은 체감 가능한 추가 가치를 제공한다.
- **M444-03 / P1:** 광고는 민감 경제/action 화면에서 계속 차단하고 유지율·신뢰·작업완료 실험 가드레일을 요구한다.
- **M444-04 / P1:** 유료 코스메틱은 WLD와 분리된 직접 entitlement이며 유료 WLD를 중간재로 사용하지 않는다.
- **M444-05 / P0:** 유료 WLD, 유료 확률형 아이템, 유료 카지노 가치, P2W 경제력, 유료 우월 WDX 정보는 계속 차단한다.
- **M444-06..08 / P1:** 스토어/결제 단위경제는 시행일·채널별로 계산하고, 구독은 간단한 셀프 해지를 유지하며, 전체 변동·운영비 차감 후 기여이익을 수익성 권위로 사용한다.
- 근거: `docs/findings/MONEYVERSE_MONETIZATION_REVENUE_RESEARCH_REVIEW_v2026.09.25.444.ko.md`, `docs/planning/deltas/v2026.09.25.444.ko.md`. 기획/문서 전용이며 runtime/Test/Production/실제매출/법적승인 완료를 주장하지 않는다.
