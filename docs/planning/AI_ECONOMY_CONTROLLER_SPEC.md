...
Runtime storage is separated from the application disk: `/srv/moneyverse-data/ai/{models,adapters,cache,datasets,evals,logs}` is the preferred local AI root. Models are not required to be resident simultaneously; local inference must use bounded concurrency.\n\n## 34. Automatic AI stock-scenario publication (v2026.09.19.261)

The existing AI newsroom may run on the hourly scheduler when AI_NEWS_AUTO_ENABLED=true and an audited operator actor UUID is configured. The model generates fictional market scenarios only; it never writes an absolute stock price. Automatic selection is narrower than manual publication: no whole-market leg, no strength-3 leg, at most two moving stocks, and a maximum 24-hour duration. Publication still goes through the existing market-event function, therefore circuit breakers, day-range bounds, idempotency, event jump parameters and drift remain authoritative. If no bounded candidate exists, the run stores the batch and publishes nothing.

### 34.1 Registered-stock-only automatic generation (v2026.09.19.285)

Automatic generation SHALL use only the currently active rows of `virtual_stocks` exposed by the authoritative AI-news context. Every automatically publishable effect must name one of those registered symbols; whole-market/null effects, unknown symbols, inactive symbols and strength-3 effects are excluded from the automatic candidate set. If there are no active registered stocks, the automatic job fails closed before the model is called. Automatic scenarios are clamped to 24 hours and may move at most two registered stocks.

The scheduler feature flag is an operational gate, not proof that the feature is working. Test and Production promotion evidence must show: the flag enabled, an auditable actor UUID configured, a successful hourly job, a stored AI batch/scenario whose effects resolve to active `virtual_stocks`, and publication through the existing market-event path. A scheduler row whose detail says `disabled` or `actor_missing` is a skipped run and must not be reported as successful scenario generation.
...
6. **Runtime proof on Test:** deploy the exact candidate SHA to the Test environment, prove backend/API/database/scheduler/AI-runtime behavior with real Test evidence, exercise success + fail-closed + rollback paths, and record exact versions/config hashes. Do not promote when any mandatory AI path remains configured but unproven.
