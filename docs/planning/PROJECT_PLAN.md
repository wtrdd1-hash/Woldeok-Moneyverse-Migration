...
### v2026.09.15.123 delta — stock discovery sorting
`/stocks` now supports shareable `sort=change|price|available|name` discovery views. Default order remains server/API order. Numeric sorting must retain canonical integer-string precision; percentage movers use exact integer cross multiplication. Invalid sort values fail back to default. Controls remain keyboard operable and expose the current choice.
