...
- **Stock comparison:** compare multiple virtual stocks with server-defined metrics on a consistent basis. The comparison route now accepts validated stock-symbol deep links from the stock detail hub so members can carry the selected stock into the comparison flow without re-selecting it.
