...
## 29. 사용자 화면 전반의 쪽지 진입점 — v2026.09.21.323
다른 사용자를 발견한 화면에서 바로 1:1 쪽지를 시작할 수 있어야 하며, 화면마다 별도 권한체계를 만들지 않고 기존 1:1 채팅 서버 권한정책을 공통으로 사용한다.

필수 1차 진입점:
- 공개 게시판 글 작성자 아이디/닉네임 영역;
- 공개 게시판 댓글·답글 작성자 영역;
- 사용자 프로필/프로필 헤더;
- 사용자 아이디·username·닉네임 검색 결과;
- 기존 회원 목록/디렉터리 중 DM 정책상 허용되는 사용자 행;
- 그 밖의 로그인 사용자 카드/칩은 화면의 사용자가 하나의 canonical 회원 ID로 확정되는 경우에만 제공.

모든 진입점은 대화 생성 전에 대상 사용자를 서버의 불변 회원 ID로 해석해야 한다. 닉네임, username, 게시글 ID, 댓글 ID는 탐색 문맥일 뿐 권한 키로 사용하지 않는다.

### 29.1 공통 동작 흐름
1. 사용자가 게시글, 댓글, 프로필, 검색결과, 회원행에서 쪽지 액션을 누른다.
2. 클라이언트는 canonical 대상 회원 ID와 선택적 비권위 source context만 전송한다.
3. 서버가 로그인 principal, 자기자신 여부, 계정상태, DM 허용설정, 차단관계, moderation 제한, 연령/안전 적격성, rollout, rate limit을 다시 검사한다.
4. 허용되면 기존 canonical 대화를 반환하거나 멱등적으로 1개만 생성한다.
5. 클라이언트는 canonical 대화화면을 열며 실제 메시지 전송 API가 커밋되기 전에는 전송완료로 표시하지 않는다.
6. 거절 시 차단 여부, 연령규칙, moderation 사유 등 상대의 민감정보를 드러내지 않는 일반 이용불가 상태를 표시한다.

### 29.2 게시판 전용 규칙
- 게시글/댓글 작성자 쪽지 버튼은 화면의 닉네임 문자열이 아니라 서버가 반환한 권위 author_user_id에 연결한다.
- 삭제·익명화·정지·시스템 작성 콘텐츠에는 오래된 쪽지 액션을 남기지 않는다.
- 회원탈퇴/익명화 뒤에도 게시물이 남는 정책이면 살아 있는 적격 회원 principal이 확인되지 않는 한 쪽지 시작 액션을 제거/비활성화한다.
- 인용/재게시 글은 사용자가 명시적으로 선택한 표시 작성자에게만 쪽지를 보내며 인용문 텍스트에서 수신자를 추정하지 않는다.
- 게시판 moderation과 DM moderation은 별도 상태이며 게시글 삭제/숨김이 기존 개인대화를 자동 삭제해서는 안 된다.

### 29.3 아이디·username·닉네임 검색
- 정확한 사용자 ID/username 검색은 서버가 공개 가능한 회원 결과를 반환한 뒤에만 쪽지 액션을 노출한다.
- 닉네임 중복을 허용하며 공개 가능한 구분정보로 사용자를 구별하고 내부적으로 canonical 회원 ID를 유지한다.
- 자유입력 닉네임만으로 클라이언트가 DM 대상 ID를 만들어서는 안 된다.
- 검색 API는 대량 회원열거 방지, rate limit, 공개 허용 필드 최소화를 적용한다.
- 이메일, 전화번호, OAuth 식별자, 공개용이 아닌 내부 ID, 잔액, 보안/제재 속성은 DM 사용자 검색키로 사용하지 않는다.

### 29.4 자기자신·차단·개인정보·이용불가 상태
- 자기자신은 UI에서 숨김/비활성화하되 서버도 self-conversation 생성을 거부한다.
- 상대 DM 비허용, 차단, 제한 또는 정책 부적격은 비민감 사유를 별도로 공개하기로 한 정책이 없는 한 같은 개인정보 안전 이용불가 표시를 사용한다.
- 로그아웃 사용자는 로그인 후 대상 문맥을 안전하게 복구하고 서버에서 다시 적격성을 검사한다.
- 기존 또는 archive된 대화가 있으면 새 대화를 만들지 않고 같은 canonical thread를 연다.

### 29.5 API 계약 추가
POST /api/v1/chat/conversations 는 { targetUserId, sourceContext? }를 받는다.
sourceContext는 profile, board_post, board_comment, member_search, member_list 같은 고정 enum의 화면 복귀/telemetry 문맥일 뿐 권한판단에는 영향을 주지 않는다.
UI 최적화가 꼭 필요한 경우에만 가벼운 eligibility endpoint를 둘 수 있으며 차단 방향, 정확한 연령검증, moderation 사유를 노출하지 않는다.

### 29.6 프론트엔드 공통 컴포넌트 계약
게시판/프로필/검색/회원목록마다 DM 규칙이 달라지지 않도록 하나의 공통 member-action primitive를 사용한다. canonical 회원 ID와 공개 가능한 표시정보를 입력받고 액션 실행 시 서버에서 항상 재검증한다.
모바일 필수: 320/360/390px에서 액션 접근 가능, 최소 44px 터치영역, 뒤로가기로 원래 게시글/검색/프로필 문맥 복귀, 가상키보드가 입력창을 가리지 않음, deep-link/back 복구가 메시지 재전송이나 대화 중복생성을 일으키지 않음.

### 29.7 악용·관측·QA 추가
QA는 게시글 작성자 -> 쪽지, 댓글 작성자 -> 쪽지, 프로필 -> 쪽지, 정확한 username/사용자ID -> 쪽지, 중복 닉네임 구분, 자기자신, 차단된 사용자쌍, DM 비허용 대상, 정지/탈퇴/익명화 작성자, 로그아웃 -> 로그인 -> 재검증, forged target ID, username/닉네임 변경 뒤 stale 게시판 데이터, 반복 대화 생성 멱등성, 모바일 back-stack/접근성, sourceContext 변조가 권한에 영향 없는지까지 포함한다.

### 29.8 구현 순서 확장
8. v2026.09.21.323-08 P0: canonical 회원 해석 계약 + 공통 DM 진입 액션 컴포넌트.
9. v2026.09.21.323-09 P0: 게시글/댓글 작성자, 프로필, 회원검색/목록 쪽지 진입점.
10. v2026.09.21.323-10 P0: 회원열거/rate-limit/개인정보/stale identity 및 cross-surface E2E를 isolated Test에서 검증.
11. v2026.09.21.323-11 P0: 작업 중간 최신기획 재확인 + exact-SHA 회귀검증 후 기존 1:1 채팅 전체 gate까지 통과한 경우에만 무중단 Production 승격.
