...
### 09:03 authoritative 재검증: P0 release/DR 유지, 세션·서비스 신뢰경계·crawler·KR economics 강화

- **Exact main / CI / writer 증거:** 시작 및 작업 중간 authoritative `main`은 `d6d2798535894c54854135c23e37d972d267baa1`(v211 병합)이다. 현재 combined commit status에는 required context가 없으며 최근 Actions는 workflow 증거일 뿐 merge-enforcement 증거로 사용하지 않는다. `CI-ENFORCE-204-01`은 trusted classifier/policy/runtime/security check가 repository-required가 되고 의도적으로 실패한 required check가 실제 merge를 막는 음성증거 전까지 **P1 / OPEN**이다. 이번 planning에는 application-data migration이 없다.
- **P0 runtime / release authority — 09:03 KST 재현:** 권위 Debian의 `moneyverse-backend`, `moneyverse-frontend`, `moneyverse-backup.timer`는 active, timer enabled, last trigger 06:26:17, next 12:22:15다. Canonical HTTPS `/api/version`은 HTTP/2 200 + `Cache-Control: no-store`, deployed application `75e69e77cdc18ef221106a008563151a4c790728`; 직전 1시간 sampled warning+ backend journal은 비어 있다. Repository main과 deployed application identity는 계속 다르다. `REL-AUTH-184-01`은 **P0 / IN PROGRESS**이며 immutable `{applicationSourceSha,imageDigest,migrationSetHash,deploymentId}`, exact-SHA Test attestation, session/API/user-flow smoke, migration equality, journal review, rollback manifest가 필요하다.
- **P0 DR — 오종결 금지:** 정기 encrypted archive 성공은 restore 증거가 아니다. `BAK-RUNTIME-177-01`은 최신 artifact를 폐기 가능한 isolated target에 restore하고 decrypt/checksum/schema+migration-set 검증, identity/session/inventory/entitlement/ledger/reward/bank/loan/stocks/casino/community/referral/audit 불변조건 대사, 실측 RPO/RTO, off-host immutable copy/retention, missed-run/verify/replication/restore alert 실제 전달이 끝날 때까지 **P0 / IN PROGRESS**다. 불일치는 fail-closed이며 destructive schema/ledger/entitlement Production 변경을 차단한다.
- **Security / auth-session-service 계약 — DIRECT ADOPT (OWASP ASVS 5.0 + API Security 2023):** browser session과 backend service identity를 별도 trust domain으로 유지한다. Security Center는 list/revoke/revoke-all을 소유하되 secret을 반환하지 않고, 민감 계정/admin/payment/economy 변경은 fresh re-auth를 요구한다. 비사용자 backend 호출은 service별 short-lived identity/certificate, issuer/audience/expiry/scope/replay 검증, DB least privilege를 사용한다. HIGH negative test는 cross-user session BOLA, stale re-auth, revoked refresh replay, CSRF mutation, wrong-audience/expired/revoked service credential, cross-service scope escalation, concurrent revoke/use다. 예방통제+masked immutable audit+alert 전달+negative test가 승격 gate이며 잔여위험에는 owner가 필요하다.
- **SEO / SEO backend — DIRECT ADOPT (Google Search Central 현행):** public community/profile/gallery/catalog/collection/search는 publication+moderation+quality SEO read-model을 통과한 경우만 index eligible이다. Canonical, robots, split sitemap+의미 있는 `lastModified`, durable redirect map, hreflang, SSR/ISR 동등 crawlable output, image metadata, visible-content parity structured data는 server-owned다. Googlebot/Search indexing, Mediapartners-Google/ad crawling, user-triggered fetcher는 별도 family로 관측하며 crawler identity가 auth/privacy/rate safety/`noindex`를 우회할 수 없다. Pagination은 stable crawlable link를 제공하고 cursor/facet은 durable landing으로 명시 승격 전 API-only 또는 noindex/canonical이다. Private/account/admin/security/transaction은 강제 noindex+sitemap 제외. Release test는 200/noindex/404/410/301/308, canonical/hreflang reciprocity, schema parity, compression/render parity, private leak, CWV를 포함한다.
- **Monetization / unit economics — DIRECT ADOPT (Google Play 현행):** universal fee를 가정하지 않는다. 발표된 KR cutover 전에는 적용 가능한 현행 programme rule이 권위이며 적격 KR alternative-billing 거래는 EEA/UK/US cohort 표가 아니라 otherwise-applicable Play service fee의 공식 감액 규칙을 사용한다. 모든 order/refund는 `{feePolicyVersion,market,effectiveDate,installCohort,transactionType,programme,billingPath,gross,platformFeeBasisPoints,billingFee,tax,refundReserve}`를 snapshot한다. 모든 shop/payment/subscription SKU는 platform/payment/tax/refund/fraud/infra/support 비용을 차감해 gross→net→contribution margin을 계산한다. 미실측 conversion/attach/repeat/ARPU/ARPDAU/ARPPU/renewal/churn/refund/CAC/LTV/payback/D1/D7/D30은 `HYPOTHESIS/TEST TARGET`; SCALE은 positive contribution+retention/fairness/security/support guardrail, bounded miss는 ITERATE, negative contribution/P2W/dark-pattern/policy breach는 KILL/HOLD다.
- **전체 기능 구현계약 / backlog:** auth/signup/login/OAuth/logout/session/security-center; profile; inventory/collection; shop/cart/payment/subscription/ad-removal; season/quest/job/level/reward; business/bank/loan; stocks/portfolio/alerts/comparison; casino; community/posts/comments/report/block; friends/clubs/invite/referral; notifications/search/gallery/upload/public content; App API; admin/audit; backup/restore; analytics/experiments; ads; SEO backend/tooling; Discord/incident 전체는 구현상태+코드/문서근거와 목적/persona/entry/screens/CTA/loading-empty-error-offline-timeout/recovery, responsive+a11y+i18n, ownership/RBAC/BOLA, endpoint/method/schema/error/idempotency/rate/resource limit, DB PK/FK/unique/check/index/transaction/concurrency, immutable audit, admin/flag/fallback/DR, privacy/abuse/security residual risk, SEO, 분석+재무 KPI, cache/performance, 완료/QA, exact-SHA deploy/rollback 요구를 유지한다. P0/P1 무결성·권한·DR·release truth가 신규 monetization/growth보다 선행한다. Planning은 runtime/DB를 배포하지 않는다.

### v212 worklog / acceptance order
최신 Google Search Central + OWASP ASVS/API Security + Google Play 공식자료 → exact GitHub main/CI → 권위 Debian services/version/journal/timer → 전체기능 security/SEO/economics delta → 작업 중 exact-main 재확인 → EN/KO 동기화 → diff/CI/PR.
