...
## 기획 사이클 — v2026.09.20.306 (2026-09-20)

### 관리자 국고 관리
- **범위:** **관리자 → 경제 → 국고 관리** 전용 기능을 기획한다. 국고는 서버 권위의 공공/경제 준비금이며 일반 회원 지갑이 아니고, 단순 “잔액 직접 설정” 기능을 허용하지 않는다.
- **권위 상세 기획서:** [ADMIN_TREASURY_MANAGEMENT_SPEC.ko.md](ADMIN_TREASURY_MANAGEMENT_SPEC.ko.md).
- **핵심 화면:** 총액/사용 가능액/예약액 요약, 불변 거래 내역, 수입, 지출, 예산 배정, 권한 보정, 대사, 준비금 정책/경고, 최소권한 내보내기.
- **회계 권위:** 모든 금전 변경은 정수 문자열 금액, 고정 참조 ID, 행위자, 멱등성 키를 가진 append-only 원장/회계 상태로만 정산한다. 관리자 화면에서 과거 원장 항목을 수정/삭제하지 않는다.
- **고위험 변경 안전장치:** 재인증, CSRF, DB 행위자 검증, 필수 사유/증거, 결정적 preview, 트랜잭션 원자성, 불변 감사로그, 저장 후 재조회가 필수다. 단일 최고관리자 환경에서는 추가 확인 challenge를 둘 수 있지만 임의로 2인 승인 모델을 만들지 않는다.
- **대사:** 차이를 탐지·표시하되 잔액을 조용히 덮어쓰지 않는다. 수정은 별도 권한과 감사가 적용되는 보정 거래로만 수행한다.
- **QA/승격:** 권한 우회, 비정상/overflow 금액, 중복 멱등성, 동시 보정, rollback, 예산 계산, 대사, 감사로그, 320/360/390px 모바일, BigInt 안전 검증을 Production 차단 조건으로 둔다. 실제 구현은 v2026.09.20.306-01~-07 순서와 exact-SHA Test → 병합 → 정확한 병합 SHA 재빌드 → 무중단 Production 승격 절차를 따른다.
- **현재 상태:** 기획/문서 전용이며 전용 국고 런타임이 이미 구현됐다고 주장하지 않는다.
