...
### 09:09 권위 기획 재조정: v212 UI 증거, P0 release/DR, service trust, SEO authority, KR economics

- **Exact main / writer / BLOCKED 해소 — P1:** 시작과 작업 중간 authoritative `main`은 `1457797823b4ba54fa373564ce30681d56b36100`이다. 이전 planning-v212가 갈라진 뒤 v212 전체 UI QA가 main에 병합되어 PR #465(`base=d6d279...`, `head=5c0054...`)는 mergeable=false가 됐다. 데이터 손실을 막기 위해 #465의 유효한 security/SEO/economics/release 계약을 현재 main 위 v213에 재통합한 뒤 stale PR을 superseded 처리한다. Main combined status는 context 0개이므로 `CI-ENFORCE-204-01`은 **P1 / OPEN**. Repository rule에 trusted change classifier + policy/runtime/security checks를 required로 두고 audited emergency bypass만 허용한다. 의도적으로 실패한 required check가 merge를 실제 차단해야 완료다. 데이터 migration 없음; 잘못된 rule만 rollback하며 direct push/bypass actor+reason/missing context/check latency를 감시한다.
- **P0 runtime/release authority — 09:08 KST 재현:** Debian backend/frontend/backup timer active, timer enabled, last 06:26:17, next 12:22:15. Canonical HTTPS `/api/version`은 HTTP/2 200, `Cache-Control: no-store`, application `75e69e77cdc18ef221106a008563151a4c790728`; 직전 1시간 warning+ backend journal은 비어 있다. Repository main과 Production application identity가 다르므로 `REL-AUTH-184-01`은 **P0 / IN PROGRESS**. Immutable `{applicationSourceSha,imageDigest,migrationSetHash,deploymentId}`, exact-SHA public-Test attestation, migration equality, auth/session/API/core-user-flow smoke, journal review, 검증된 rollback manifest가 모두 필요하며 identity 누락/불일치는 fail-closed다.
- **P0 backup/restore — 오종결 금지:** timer/archive 정상은 restore 증거가 아니다. `BAK-RUNTIME-177-01`은 최신 encrypted artifact를 폐기 가능한 isolated target에 restore하고 decrypt/checksum/schema+migration-set equality, identity/session/inventory/entitlement/ledger/reward/bank/loan/stocks/casino/community/referral/audit invariant 대사, RPO/RTO 실측, off-host immutable retention, missed-run/verify/replication/restore alert 실제 전달까지 **P0 / IN PROGRESS**다. Destructive schema/ledger/entitlement Production 변경은 차단하며 restore test는 Production을 대상으로 하지 않는다.
- **v212 UI/CSP QA는 release input이지 Production 증거가 아니다:** main에는 280px overflow, shop 16px input, title 조합, Gallery AdSense CSP 최소허용 수정과 targeted 15/15, frontend 71 files/623 tests, typecheck/build, 300 responsive browser checks 증거가 있다. 상태는 **IMPLEMENTED / NOT YET PRODUCTION-ATTESTED**. Exact main을 public Test에 올린 뒤 deletion/legal/shop/gallery를 280/320/390+desktop에서 CSP console, keyboard/focus, zoom, title/canonical, ad-disabled fail-closed까지 동일 source로 검증해야 한다. Frontend rollback은 직전 artifact를 보존하며 backend/DB는 재시작하지 않는다.
- **Security/auth/session/service boundary — DIRECT ADOPT (OWASP ASVS 5.0/API Security):** browser session과 non-user backend identity는 별도 trust domain이다. Security Center는 `GET /sessions`, `DELETE /sessions/:id`, `POST /sessions/revoke-all`을 소유하고 opaque id/device label/timestamps/current만 반환한다. 이메일/전화/MFA/recovery/admin/payment/high-value economy mutation은 fresh re-auth가 필요하다. Auth/payment/reward/bank/stock/casino/admin worker는 개별 service account, short-lived token/certificate, issuer/audience/expiry/scope/replay 검사와 least-privilege DB role을 사용하며 privileged shared/static credential을 금지한다. Cross-user BOLA, CSRF, stale re-auth, revoked-refresh replay, concurrent revoke/use, wrong-audience/expired/revoked service credential, cross-service scope escalation, reward/payment replay는 HIGH blocker다. 예방통제+immutable masked audit+실제 alert+negative test가 승격조건이며 예외에는 owner/expiry/flag/rollback이 필요하다.
- **SEO / SEO backend — DIRECT ADOPT (Google Search Central 2026-09-17까지 현행):** public profile/community/gallery/catalog/collection/search는 server-owned publication+moderation+quality read-model을 통과해야 index eligible이다. Stable slug/canonical owner, 의미 있는 `updatedAt/lastModified`, locale/hreflang, redirect history를 저장한다. Split sitemap은 durable 200-indexable row만 포함하며 private/account/security/admin/transaction/unresolved moderation은 noindex+sitemap 제외다. Pagination은 stable crawlable link, cursor/facet/query variant는 명시적 durable landing 승격 전 API-only 또는 noindex+canonical이다. Structured data는 visible content/지원 type과 일치하고 image dimensions/alt/metadata를 제공한다. SSR/ISR/cache variant는 title/H1/canonical/robots/hreflang/schema parity를 유지한다. Googlebot/Search, Mediapartners-Google/ads, user-triggered fetcher는 개인정보 query/body 없이 별도 관측한다. QA는 200/noindex/404/410/301/308, redirect loop, canonical/hreflang reciprocity, sitemap lastModified, compressed-render parity, private leak, CWV를 포함한다. KPI는 impression→CTR→organic visit→signup→activation→D7/D30→revenue이며 crawl/index error와 organic CAC가 guardrail이다.
- **Shop/payment/subscription unit economics — DIRECT ADOPT (Google Play 현행):** universal fee를 금지한다. Global rollout 전 remaining-market auto-renew subscription은 현재 15%, 15% tier는 연 USD 1M까지 15%/초과 30%, 적격 한국 alternative billing은 otherwise-applicable Play service fee에서 4 percentage points 감소한다. EEA/UK/US new/existing-install 표와 혼합하지 않는다. Order/refund는 `{feePolicyVersion,market,effectiveDate,installCohort,transactionType,programme,billingPath,gross,platformFeeBasisPoints,billingFee,tax,refundReserve}`를 entitlement authority와 독립 snapshot한다. 모든 consumable/non-consumable/subscription/ad-removal SKU는 platform/payment/tax/refund/fraud/infra/storage/CDN/notification/support/moderation, 할인 cannibalization, repeat/renewal을 반영해 gross→net→contribution을 계산한다. 미실측 attach/conversion/ARPU/ARPDAU/ARPPU/churn/refund/CAC/LTV/payback/D1/D7/D30은 `HYPOTHESIS/TEST TARGET`; positive contribution+retention/fairness/security/support guardrail이면 SCALE, 제한적 miss는 ITERATE, negative contribution/P2W/dark-pattern/policy breach는 KILL/HOLD다.
- **전체 기능 실행 backlog / 우선순위:** auth/signup/login/OAuth/logout/session/security-center; profile; inventory/collection; shop/cart/payment/subscription/ad-removal; season/quest/job/level/reward; business/bank/loan; stocks/portfolio/alerts/comparison; casino; community/posts/comments/report/block; friends/clubs/invite/referral; notifications/search/gallery/upload/public content; App API; admin/audit; backup/restore; analytics/experiments; ads; SEO backend/tooling; Discord/incident 전체는 구현상태+코드/문서근거, 목적/persona/entry/screen/CTA/loading-empty-error-offline-timeout/recovery, responsive/a11y/i18n, ownership/RBAC/BOLA, API schema/errors/idempotency/rate/resource limit, DB PK/FK/unique/check/index/transaction/concurrency, audit/flag/fallback/DR/privacy/abuse, SEO, 분석+재무 KPI, cache/performance, unit/integration/E2E/real-DB/security/regression, exact-SHA Test→main→Production smoke/rollback 계약을 유지한다. 순서는 P0 DR/release truth → real-DB/economy integrity → HIGH auth/payment/economy/service/upstream → P1 merge enforcement/Production responsive proof → correctness → monetization → SEO/acquisition → retention/accessibility. Planning은 runtime/DB를 배포하지 않는다.

### v213 worklog / 수용 순서
최신 Google Search Central + OWASP ASVS 5.0 + Google Play 공식자료 → exact GitHub main/open PR/CI와 v212 code+QA 증거 → 권위 Debian services/version/journal/timer → superseded #465의 기획결정을 데이터손실 없이 current-main UI 증거와 통합 → 전체기능 security/SEO/economics/QA delta → 작업 중 exact-main 재확인 → EN/KO 동기화 → diff/CI/PR.
