...
- **근거 범위:** OpenAlex/Crossref 후보 35,429건을 수집해 DOI 우선/제목 보조 중복 제거 후 **고유 탐색 후보 33,341건**을 구축했다. 이는 전수 원문 수작업 검토 주장이 아니며 실제 채택은 공식/동료심사 근거 집중 검토를 사용한다.
- **분산 포트폴리오:** 무료 핵심 + 맥락형 공개광고 + `Moneyverse Plus` 광고제거/편의/표현 가치 + 비P2W 코스메틱 직접 entitlement + 명확한 스폰서십을 수익화 기본축으로 한다. creator/B2B/API는 별도 게이트가 있는 후기 단계다.
- **무료 핵심 불변조건:** 결제를 강요하려고 필수 경제·게임·커뮤니티 참여를 의도적으로 악화하지 않는다. 프리미엄은 무료 티어를 망가뜨려 만든 결핍이 아니라 체감 가능한 추가 가치를 제공해야 한다.
- **광고 경계:** 지갑, 송금, 대출, WDX 주문/판단, 카지노, 계정보안, 결제실패, 관리자 action 화면에는 광고를 넣지 않는다. 광고 실험은 홀드아웃 D1/D7/D30, 작업완료, 불만/오클릭, 지연, 신뢰 가드레일을 요구한다.
- **직접 유료상품:** 코스메틱/표현 상품은 WLD 원장과 분리된 실화폐 entitlement로 직접 판매한다. 유료 WLD를 중간재로 요구하지 않는다.
- **P0 금지 수익화:** `WLD_CASH_PURCHASE`, `PAID_RANDOM_ITEM`, 유료 카지노 가치/베팅, P2W 경제력, 유료 우월 WDX 정보, 돈을 내면 빨라지는 모더레이션은 계속 차단한다.
- **시행일 기준 단위경제:** 시장, 채널, 시행일, 해당 시 설치코호트, 거래유형, 결제경로, 프로그램별로 원가·수수료를 버전 관리한다. 미래 스토어 요율을 시행 전에 적용하면 안 된다.
- **수익성 권위:** 총매출이 아니라 스토어/플랫폼/PSP 수수료, 세금, 환불/차지백, 수익배분, 증분 인프라, 지원/모더레이션, 현지화/콘텐츠, 컴플라이언스/법무 운영비를 차감한 기여이익을 최적화한다.
- **실험 진실성:** 가격, 전환율, eCPM, churn, CAC, LTV, 매출예측은 Moneyverse 실측 전까지 `HYPOTHESIS/TEST TARGET`이다. 가드레일 악화 시 실험을 중단하거나 롤백한다.
- 상세 근거: `docs/findings/MONEYVERSE_MONETIZATION_REVENUE_RESEARCH_REVIEW_v2026.09.25.444.ko.md`; 델타: `docs/planning/deltas/v2026.09.25.444.ko.md`. 기획/문서 전용이며 결제 구현, Test, Production 완료를 주장하지 않는다.
