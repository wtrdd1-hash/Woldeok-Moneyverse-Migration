...
기능별 구현·QA·운영 증거의 현재 상태와 P0/P1 차단 항목은
[기획 공백 해소 및 수용증거 명세](PLANNING_GAP_CLOSURE_SPEC.ko.md)를 따른다.
이 링크는 과거 기록을 삭제하지 않으며, 현재 상태를 역사적 worklog와 혼동하지 않기 위한
권위 register를 제공한다.
...
- **근거 범위:** OpenAlex/Crossref 후보 35,429건을 DOI 우선/제목 보조 중복 제거해 **고유 탐색 후보 33,341건**으로 만들었으며, 이는 전수 원문 수작업 검토 주장이 아니다.
- **포트폴리오와 무료 핵심:** 맥락형 공개광고, 광고제거/편의/표현 구독, 비P2W 코스메틱 직접 entitlement, 명확한 스폰서십을 사용할 수 있으나 결제를 강요하려고 필수 경제·게임·커뮤니티 참여를 악화하지 않는다.
- **하드 경계:** 지갑, 송금, 대출, WDX, 카지노, 계정보안, 결제실패, 관리자 action 화면에는 광고를 넣지 않는다. 유료 WLD, 유료 확률형 아이템, 유료 카지노 가치/베팅, P2W 경제력, 유료 우월 WDX 정보, 돈을 내면 빨라지는 모더레이션은 금지한다.
- **경제적 진실성:** 시행일/채널별 단위경제를 쓰고 수수료, 세금, 환불, 차지백, 수익배분, 인프라, 지원, 모더레이션, 현지화, 콘텐츠, 컴플라이언스/법무 비용 차감 후 기여이익을 최적화한다. 실측 전 가격·전환율·eCPM·churn·CAC·LTV·매출은 `HYPOTHESIS/TEST TARGET`다.
- 상세 근거: [수익화 조사 검토](../findings/MONEYVERSE_MONETIZATION_REVENUE_RESEARCH_REVIEW_v2026.09.25.444.ko.md)와 기획 델타. 기획/문서 전용이며 결제 구현, Test, Production 완료를 주장하지 않는다.

## Moneyverse Arcade 조사 경계 — v2026.09.26.444

- 계획된 Arcade는 선택적 비도박 오락 화면이며 보상은 현금화·양도할 수 없다. 유료 입장, 현금화, 베팅, 유료 랜덤 보상, WLD 구매, 경제적 우위를 도입하면 안 된다.
- 18,578건 검증 탐색 corpus와 상세 구현/릴리스 제약은 [Arcade 명세](MONEYVERSE_ARCADE_GAME_SPEC.ko.md)에 기록했다. 기획/조사 전용이며 구현에는 별도 exact-SHA Test 증거가 필요하다.
...
- **근거 식별:** v400의 중복 제거된 31,289건 탐색 corpus를 유지하고 미러·번역본·추적 파라미터 URL 변형·이미 채택한 표준을 다시 세지 않는다. DOI/canonical URL/제목+발행기관+버전을 중복 제거 키로 사용한다.
- **P0 유상 직업 pacing:** 일반 참여 무제한은 즉시 발행을 뜻하지 않는다. 모든 보상 작업은 서버 권위 `eligible_at` 또는 독립 검증 가능한 완료 경계를 가져야 하며 클라이언트 클릭만으로 즉시 사용 가능한 WLD를 만들 수 없다.
- **P0 원자적 원장:** 직업 보상, 송금, 세금, 시장 정산, 국고 지급, 환불은 고유 business idempotency key를 가진 원자적 영속 잔액 mutation이다. Serializable 재시도, row lock 또는 동등한 invariant 보존 설계를 사용하고 serialization/deadlock 실패 시 transaction 전체를 재시도한다.
- **P1 경제 제어 지표묶음:** gross/net 발행, sink, active money supply, dormant balance, velocity, 품목군 가격지수, 신규/중간 유저 구매력, 자산집중, 거래량을 함께 본다. 자동 파라미터 변경은 변화율 제한·버전관리·가역·감사가 가능해야 한다.
- **P1 개입 검증:** 거래세와 item sink는 안정화가 보장된 해법이 아니라 가설이다. 영구 적용 전에 가격·거래량·희소성·대체수요·구매력·분배·어뷰징 효과를 검증한다.
- **P1 정산/유동성:** 국고·은행·시장 정산주기는 유동성 비용, netting 이익, 거래상대방/운영 위험, 복구동작을 함께 고려하며 latency 최소화만 목표로 하지 않는다.
- **P1 관측성:** 경제 trace/metric/log를 연계하고 정책버전별 발행/sink/velocity/세금/국고/retry/중복차단/rollback telemetry를 남긴다.
- **접근성 정정:** WCAG 2.2 SC 2.5.8 AA 규범 target-size 최소는 정의된 예외가 있는 24x24 CSS px이다. Moneyverse는 주요 터치 컨트롤에 더 강한 제품 규칙인 >=44x44 CSS px를 계속 적용한다. 44px를 WCAG AA 최소라고 표현한 문구는 이 절이 supersede한다.
- **운영:** Test/Production 배포환경을 분리·직렬화하며 실제 호스트 증거가 13.7 이상 업데이트를 입증하기 전에는 관측된 Debian 13.6을 runtime 사실로 유지한다.
- **모바일 상거래:** 실제 결제 IAP로 Moneyverse 가상화폐를 구매하게 되면 플랫폼 정책을 release contract에 포함한다. Apple IAP 구매 게임 화폐는 만료시키지 않고 구매형 무작위 가상아이템은 구매 전 확률 공개를 요구한다.
- 상세 권위: [deltas/v2026.09.24.433.ko.md](deltas/v2026.09.24.433.ko.md). 기획/문서 전용이며 런타임 구현·Test·Production 완료를 주장하지 않는다.
...
- **P1 44px 최소 터치 타겟 & 하단 안전 여백 (UX):** Apple HIG 및 WCAG 2.2 AA 기준을 충족하는 44px 터치 영역과 `pb-[calc(env(safe-area-inset-bottom)+5rem)]` 클리핑 방지 레이아웃을 강제한다.
- **상세 명세:** [deltas/v2026.09.23.406.ko.md](deltas/v2026.09.23.406.ko.md).
...
- **live/real-time 의미:** 홈/신문/developer 데이터는 source/fetch timestamp, 최대 staleness, stale UI, retry/backoff, safe fallback을 정의해야 한다. client animation만으로 실시간을 증명하지 않는다.
- **거버넌스:** 영문을 기준 언어, 한국어를 동기화된 제2언어로 유지한다. 모든 기획 회차는 이 통합 기획을 갱신하고 구현 직전 및 작업 중간 remote main을 재확인하며 GitHub Markdown 표준과 기획/구현/배포 증거 구분을 유지한다.
- **v348 수용:** 영/한 버전 및 의미 동기화. 신규/변경 런타임 기능은 권위, 데이터/API 계약, 실패·부정 경로, 개인정보/보안, 접근성/반응형, 관측성, QA, 릴리스, 롤백 계약을 가져야 한다. 구현 직전과 작업 중간 living plan을 다시 읽는다.
...
### v2026.09.26.443 — 대한민국 법률 출시 게이트
- 상세 권위: `KR_LEGAL_COMPLIANCE_AUDIT.ko.md`. 실제 등급/GRAC + 19+ + 법률/채널 증거 전 대한민국 카지노/확률 기능은 fail-closed하며 근거 없는 승인/인증 문구는 P0 결함이다.
- 운영 광고가 이미 존재하므로 사업자/세무 상태와 광고·개인정보·국외이전 증거는 미래 과제가 아니라 현재 운영 게이트다.
- 실결제 디지털상품/구독/광고제거는 판매자 신원/신고, 거래조건, 청약철회/환불/해지, 미성년자 계약, 결제사업자 대사 통과 전 BLOCK.
- `ageConfirmed` 자기확인을 검증된 연령으로 표현하지 않는다. 광고성 메시지는 국내 동의/suppression/야간 통제 전 BLOCK. WLD/게임결과 현금환전은 새 법률검토 전 금지한다.
