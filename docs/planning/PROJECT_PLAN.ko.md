...
> **최초 기준:** 2026-08-26
> **현재 통합 버전:** v2026.09.18.202
> **구현·증거 동기화:** 2026-09-18
> **영문 기준 문서:** [PROJECT_PLAN.md](PROJECT_PLAN.md)

과거 상세 변경은 Git 이력과 버전별 changelog/worklog에서 복구할 수 있다. 이 문서는 현재 구현을 위한 권위 계약이다. 다른 개발자나 AI가 과거 초안을 현재 사실로 추정하지 않고 이 문서만으로 기능 범위, 권위 경계, 사용자 상태, API, 영속화, 보안, SEO, 사업성, QA, 릴리스 게이트와 롤백 조건을 이해할 수 있어야 한다.

## 회차 델타 — v2026.09.18.202 (2026-09-18)

### 최신 백업 증거, 반응형 candidate 대조, 광고 CSP·릴리스 게이트

- **정확한 main / 동시 candidate:** 시작과 작업 중간 authoritative main은 `bc988400c13b35172490b305564de5bfc6712112`다. PR #449 (`a476e353bfc3cd792a18748e61aedf40773e1182`)는 **OPEN / MERGEABLE / CI IN PROGRESS**이며 현재 main이나 Production으로 기록하지 않는다. 범위는 viewport-safe drawer, 5개 모바일 하단 탭 균등폭, 좁은 은행 화면 stack/wrap, 480px 미만 1열 요약, inventory shrink guard와 회귀검증이다. Branch 증거는 frontend 70파일/619 tests, Production build, Chromium 320/360/390px 21/21 HTTP 200·document-level overflow 0이다. Runtime-relevant이므로 exact-candidate CI → isolated Test backend/API/frontend/user-flow smoke → main merge → Production promotion → responsive/session/ad 사후 smoke → 이상 시 rollback 순서를 강제한다. DB/API/auth/ledger/entitlement migration은 없다.
- **P0 백업/DR `BAK-RUNTIME-177-01`: IN PROGRESS, 2026-09-18 00:22:17~00:22:19 KST 최신 성공 실행.** 권위 Debian의 backend/frontend/backup timer는 active, timer enabled, 다음 실행은 06:26 KST다. 새 encrypted archive `moneyverse-debian13-20260917T152217Z.tar.zst.enc`, `database.dump`, `photos.tar.zst`, `manifest.txt`가 모두 OK이고 service도 성공 종료했다. 이는 RPO 증거를 개선하지만 복구 가능성을 증명하지 않는다. DONE에는 최신 archive isolated restore, decrypt/checksum/schema/migration-set 검증, identity/session·inventory/entitlement·ledger/reward·bank/loan·stocks·casino·community/referral·audit domain reconciliation, 실측 RPO/RTO, off-host immutable 복제/retention, missed-run/verify/replication/restore 실패 alert 전달 증거가 모두 필요하다. 불일치는 fail-closed이며 그 전 destructive schema/ledger/entitlement rewrite는 Production 차단이다.
- **Runtime identity / 관측성:** 00:58 KST backend 직접 `/api/version`은 HTTP 200, `Cache-Control: no-store`, deployed application build `75e69e77cdc18ef221106a008563151a4c790728`을 유지하며 직전 1시간 warning 이상 backend journal은 비어 있다. Repository main과 deployed application identity는 별도 권위다. `REL-AUTH-184-01`은 CONTROL_PLANE_ONLY ×3, RUNTIME_RELEVANT exact-candidate ×2, MIXED ×1, missing/stale/foreign-candidate 음성 증거 전까지 **P0 / IN PROGRESS**다.
- **반응형/접근성 이슈 `UI-RESP-202-01`: P1 / IN PROGRESS.** 최초 기록 2026-09-18 PR #449 audit. 영향은 global mobile drawer/bottom nav, bank cards/actions/debt summary, inventory profile/quick slots이다. 원인은 좁은 viewport에서 fixed/min-width 및 과밀 horizontal layout 가정이다. 수정설계는 candidate의 viewport cap/grid/shrink/stack/wrap이며 migration 없음, rollback은 이전 frontend release다. Component regression, 320/360/390px Chromium, keyboard/focus order, 200% zoom/reflow, 명세된 최소 44px touch target, 인증된 bank/inventory state, 긴 KO/EN label, loading/error/empty, document-level overflow 0을 Test에서 검증한다. Production 수용에는 실기기 smoke와 CWV/error-rate 비교를 추가한다.
- **광고/CSP 이슈 `ADS-CSP-202-01`: P2 / OPEN.** PR #449 browser QA에서 Gallery의 기존 Google ad-quality `sodar2.js` CSP 거부가 관측됐다. CSP를 광범위하게 완화하지 않는다. Exact main/Test에서 console + response CSP + network initiator로 재현하고 해당 요청이 실제 구성 광고 제품에 필수인지 확인한다. 필수라면 공식 문서의 정확한 origin/directive만 allow하고 XSS/security-header 회귀를 수행한다. 선택적이면 계속 차단한다. ad request→fill→provider impression→revenue, CSP violation 수, Gallery 오류/세션이탈을 관측한다. 실제 광고 렌더링/측정 또는 사용자 흐름을 깨는 경우에만 Production 차단으로 승격한다.
- **전체 기능 실행계약:** auth/signup/login/OAuth/logout/session/security center, profile, inventory/collection, shop/cart/payment/subscription/ad-removal, season/quest/job/level/reward, business/bank/loan, virtual stocks/portfolio/alerts/comparison, casino/randomized mechanics, community/posts/comments/report/block, friends/clubs/invite/referral, notifications/search/gallery/upload/public content, App API, admin/audit, backup/restore, analytics/experiments, ads, SEO backend/tooling, Discord, incident operations는 화면 entry/CTA/loading-empty-error/offline/timeout/recovery, responsive/a11y/i18n, ownership/RBAC/BOLA, endpoint/request/response/error/idempotency/rate-limit, service rule, DB PK/FK/index/unique/check/transaction/concurrency, immutable audit/observability, admin/flag/fallback, privacy/abuse, SEO, event/KPI, performance/cache, unit/integration/E2E/real-DB/security/regression, deploy/rollback 계약을 유지한다. 증거는 환경별로 관리하며 open candidate는 merge 및 exact-environment 수용 전 canonical 구현상태를 승격하지 않는다.
- **SEO/SEO 백엔드 — 최신 공식자료 직접채택:** Google Search Central 최신 major update는 2026-09-17 infinite-scroll 지침 이전(내용 변경 없음)이며 9월16일 Search profile badge, 9월8일 regional Search-experience 문서가 뒤따른다. Public community/catalog/collection/search는 crawlable pagination URL, server-renderable link, stable ordering, self-canonical/title/H1, 정상 200/404, 통제된 sitemap 포함을 유지하고 cursor/facet은 SEO read-model 승격 전 기본 noindex/canonical이다. Metadata/canonical/robots/sitemap+`lastModified`/breadcrumb/JSON-LD/hreflang/SSR·ISR/image/redirect/crawler log/Search Console·Naver ingestion은 server 책임이며 private/account/admin/security/transaction은 강제 `noindex`다. KPI는 impression→CTR→organic visit→signup→activation→D7/D30→revenue, guardrail은 CWV/thin/duplicate index다.
- **보안 — 최신 공식자료 직접채택:** OWASP API Security 2023을 최신 API-specific Top 10 기준으로, 기존 ASVS 5를 application verification 기준으로 유지한다. BOLA, broken authentication, object-property/function authorization, resource exhaustion, sensitive-business-flow abuse, SSRF, misconfiguration, API inventory, unsafe upstream consumption을 release-block한다. CSRF, XSS/SQLi/path/command injection, upload isolation, brute force/bot/multi-account/referral fraud, reward/ledger/payment replay-safe idempotency, signed receipt/webhook, CORS/CSP/security header, secret/supply-chain, DB least privilege, backup-key separation, masked immutable audit, retention/deletion도 전 기능 gate다. CSP 변경은 최소 directive와 XSS 음성 회귀가 필수다.
- **수익성/unit economics — Play 정책 최신 재확인:** Google Play EEA/UK/US standard 예시는 auto-renew subscription 10%, 기타 new-install 20%, 기타 existing-install 25%와 applicable 5% billing fee를 유지하며 universal fee 가정은 금지한다. 각 SKU는 `{market,effectiveDate,installCohort,transactionType,programme,billingPath,tax/refund assumption}`을 먼저 resolve한 뒤 revenue→net revenue→gross/contribution margin을 계산한다. 미실측 ARPU/ARPDAU/ARPPU, conversion/attach/repeat, renewal/churn/refund, CAC/LTV/payback, ad eCPM/fill/CTR/ad-induced churn, infra/support/fraud cost, D1/D7/D30은 `HYPOTHESIS/TEST TARGET`만 허용한다. Gallery 광고는 raw request 수가 아니라 `ADS-CSP-202-01` 해소 전후 순광고매출과 retention으로 판단한다.
- **우선순위/실행 백로그:** P0 isolated restore + off-host immutable replication + alerts → P0 잔여 release-class/candidate-authority matrix → migration-204 real-DB/economy-integrity → HIGH auth/BOLA/CSRF/idempotency/ledger/payment/webhook/abuse → P1 exact-candidate responsive 수용 + required-check enforcement → Discord Production smoke → P2 ad-CSP 판정 → core correctness → monetization → SEO/acquisition → retention/accessibility. Planning은 runtime/DB를 배포하지 않는다.

### v202 worklog / 수용 순서
Google Search Central·Google Play·OWASP 최신 공식자료 → exact main/canonical 확인 → 권위 Debian service/version/00:22 backup 재현 → open PR #449/CI/QA 대조 → 전체 기능/SEO/보안/사업성 delta → 작업 중간 exact-main 재확인 → EN/KO 동기화 → diff/branch/PR.
