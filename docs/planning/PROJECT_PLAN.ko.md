...
- P3 첫 구현 후보 `v2026.09.12.14`는 관리자 전용 읽기 전용 Economy Scenario Lab이다. 현재 M2와 최근 24시간 발행/소각을 이용한 결정론적 계산만 수행하고 정책·원장·잔액·DB 상태를 쓰지 않는다. 정확한 SHA의 Test 게이트를 통과하기 전에는 Production에 반영하지 않으며 추천/자동 정책 기능은 아직 구현하지 않는다.
