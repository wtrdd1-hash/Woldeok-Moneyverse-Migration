...
### 05:06 권위 런타임/CI 갱신, planning-only release-build 경계 명확화

- **정확한 main / CI:** 시작·중간 authoritative `main`은 `bf392f69ff49ad0cf4078f763879b3776a5f69a8`이며 EN/KO v206이 동기화돼 있다. Branch protection은 `protected=true`지만 `required_status_checks.enforcement_level=off`, contexts/checks 0개이므로 `CI-ENFORCE-204-01`은 **P1 / OPEN**이다. 최신 main `Build Production Release` run `35268166090`은 planning-only SHA에서 success지만 canonical Production `/api/version`은 계속 application `75e69e77cdc18ef221106a008563151a4c790728`을 반환한다. 따라서 release-build 성공은 build 증거일 뿐 배포/승격 증거가 아니다. Change-class required check, exact candidate attestation, failing-check merge 차단 negative test를 완료하고 bypass/direct-push/check latency 및 build→deploy identity mismatch를 감시한다.
- **런타임 / DR — P0:** 05:06 KST Debian backend/frontend/`moneyverse-backup.timer`는 active, timer enabled, last `00:22:17`, next `06:26:00`, sampled backend warning+ journal은 0건이다. Canonical HTTPS `/api/version`은 HTTP/2 200, `Cache-Control: no-store`, deployed application `75e69e77cdc18ef221106a008563151a4c790728`이다. `BAK-RUNTIME-177-01`은 **P0 / IN PROGRESS**다. 예약 백업 존재는 restore 증명이 아니며 newest encrypted archive isolated restore→decrypt/checksum/schema/migration-set 검증→identity/session/inventory/entitlement/ledger/reward/bank/loan/stocks/casino/community/referral/audit reconciliation→실측 RPO/RTO→off-host immutable replication+retention→missed-run/verify/replication/restore-failure alert 실제 전달까지 필요하다. 통과 전 destructive state/economy 변경은 차단한다.
- **Release authority — P0:** `REL-AUTH-184-01`은 **IN PROGRESS**다. repository main `bf392f69...`, successful release-build, deployed application `75e69e77...`의 분리는 promotion이 의도적으로 분리된 경우에만 정상이다. Release 증거는 `{applicationSourceSha,imageDigest,migrationSetHash,deploymentId}`를 묶고 Test exact-SHA, DB migration equality, session continuity, API/user-flow smoke, journal review, rollback manifest를 기록해야 한다. missing/stale/foreign identity는 fail-closed다.
- **SEO/SEO backend — 직접채택:** Google Search Central 최신 update log는 2026-09-17 infinite-scroll guidance 이전에 guidance 변경이 없다고 기록한다. Public community/catalog/collection/search는 crawlable pagination URL/server-renderable link, stable ordering, self-canonical/title/H1, 정확한 200/404/410/301/308, durable-only sitemap+`lastModified`, breadcrumb/적합한 structured data, hreflang, image metadata, SSR/ISR/CWV budget을 유지한다. Cursor/facet은 public SEO read-model이 durable landing으로 승격하지 않는 한 API-only 또는 noindex/canonical이다. SEO backend는 metadata/canonical/robots/sitemap/redirect serializer, crawler log, Search Console/Naver ingestion, index-error reporting을 소유하고 account/admin/security/transaction은 noindex를 강제한다. KPI는 impression→CTR→organic visit→signup→activation→D7/D30→revenue이며 thin/duplicate/UGC/CWV guardrail을 둔다.
- **보안 — 직접채택:** OWASP API Security Top 10 2023을 API 기준선으로 유지한다. Object/function/property 접근은 서버 ownership/RBAC, auth/session/OAuth mutation은 replay/CSRF/session 통제, resource-expensive/sensitive flow는 quota/rate/bot/fraud 통제, SSRF/upstream은 destination policy·TLS·schema validation/sanitization·redirect/timeout/body/concurrency bound·circuit breaker를 요구한다. 경제/payment/reward write는 server-authoritative value, idempotency key, signed receipt/webhook, DB least privilege, immutable masked audit를 요구한다. HIGH는 예방+탐지 통제, negative test, alert 전달, residual-risk owner 전까지 승격 차단이다.
- **수익성/unit economics — 직접채택:** Google Play 현행 문서는 EEA/UK/US standard 예시로 auto-renew 10%, 기타 new-install 20%, 기타 existing-install 25%, Play Billing 적용 시 5% billing fee를 제시하며 나머지 시장은 rollout 전 해당 현행 schedule을 유지한다. 모든 shop/payment/subscription SKU는 `{market,effectiveDate,installCohort,transactionType,programme,billingPath,tax/refund}`를 resolve한 뒤 gross→net→contribution margin을 계산한다. 미실측 revenue/net revenue/gross margin/contribution margin/ARPU/ARPDAU/ARPPU/attach/repeat/renewal/churn/refund/eCPM/fill/CTR/CAC/LTV/payback/infra/support/fraud/D1-D7-D30은 `가설/테스트 기준`이다. Contribution margin과 retention/fairness/support/fraud guardrail을 통과하면 scale, 혼합 증거면 iterate, 지속 음의 contribution·정책위반·악용가능성·유의한 retention 악화면 kill한다.
- **전체 기능 실행계약 / backlog:** auth/signup/login/OAuth/logout/session/security center, profile, inventory/collection, shop/cart/payment/subscription/ad-removal, season/quest/job/level/reward, business/bank/loan, stocks/portfolio/alerts/comparison, casino/randomized mechanics, community/posts/comments/report/block, friends/clubs/invite/referral, notifications/search/gallery/upload/public content, App API, admin/audit, backup/restore, analytics/experiments, ads, SEO backend/tooling, Discord/incident 전체는 구현상태+코드/문서 근거, entry/CTA/loading-empty-error-offline-timeout/recovery, responsive/a11y/i18n, ownership/RBAC/BOLA, API request/response/error/idempotency/rate-limit, service rule, DB PK/FK/unique/check/index/transaction/concurrency, immutable audit/observability, admin/feature flag/fallback, privacy/abuse, SEO/event/KPI/performance/cache, unit/integration/E2E/real-DB/security/regression, deploy/rollback 증거를 유지한다. 우선순위는 P0 DR → P0 release authority → real-DB/economy integrity → HIGH auth/economic/payment/upstream → P1 CI enforcement/responsive Production proof → correctness/monetization/SEO/growth이다. 기획 변경은 runtime/DB를 바꾸지 않는다.

### v207 worklog / 수용 순서
최신 Google Search Central + OWASP API + Google Play 공식자료 → exact main/canonical/protection/Actions → 권위 Debian services/version/timer/journal → 전체 기능/security/SEO/economics/QA delta → 작업 중간 exact-main 재확인 → EN/KO 동기화 → diff/commit/PR/CI.
