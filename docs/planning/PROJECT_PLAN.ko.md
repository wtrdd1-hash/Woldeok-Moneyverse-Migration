...
### 11:03 권위 복구; DNS 관측성 이슈; crawler/indexing·service-auth·한국 수익성 갱신

- **writer/main 복구 — DONE:** 직전 v215 PR #469는 exact head `bcbd6421b7fece85eeb13564ab20333d4339d1d4`의 CI run `35293816409`가 성공했지만 병합 없이 닫혀 있었다. base `d87dfea...`가 변하지 않았고 `mergeable=true`임을 다시 확인한 뒤에만 reopen하여 exact-head squash merge했다. 새 authoritative main은 `281dea15b828204508b75aec3ced4c9eb1b38bbb`이며 v216 branch는 정확히 여기서 시작한다. 두 언어 문서를 덮어쓰지 않고 반복 planning-sync BLOCKED를 해소했다.
- **HIGH OPS-DNS-216-01 — TODO / 2026-09-18 11:03 KST 재현:** 권위 Debian에서 `curl https://woldeok.com/api/version`이 `Could not resolve host`, `getent hosts woldeok.com`도 주소 없음이었지만 direct backend `http://127.0.0.1:3002/api/version`은 200, `Cache-Control: no-store`, backend SHA `75e69e77cdc18ef221106a008563151a4c790728`이었다. backend/frontend/backup timer는 active이고 backend warning journal은 비어 있었다. 영향은 host-local 외부의존성 점검, 인증서/edge smoke, webhook 및 DNS가 필요한 runtime 경로다. 원인은 아직 확정하지 않고 resolver/network/upstream DNS를 가설로 둔다. 인프라 작업은 `/etc/resolv.conf`, resolver route/reachability, 독립 vantage의 authoritative/public DNS 증거 수집, 개인정보 payload 없는 `{resolver,hostname,result,latency,rcode}` probe, NXDOMAIN/SERVFAIL/timeout/local-resolver 분리, 제한 재시도+독립 확인 후 alert이다. DB migration 없음. resolver/config 변경은 사전 캡처 상태로 rollback한다. 테스트는 local+독립 public resolution, canonical HTTPS `/health`·`/api/version`, outbound webhook/provider DNS, retry/backoff이며 analytics 비핵심만 fail-open, payment/auth destination은 fail-closed다. Test 수용조건은 두 resolver/vantage에서 30회 연속 DNS+TLS 성공 및 false NXDOMAIN 0건, Production은 canonical DNS/TLS 불가 또는 payment/auth provider resolution 미검증이면 차단한다. DNS error rate, p50/p95, rcode, HTTPS smoke, provider resolution을 관측한다.
- **P0 DR/runtime 증거:** 06:26:17→06:26:19 encrypted backup은 archive, `database.dump`, `photos.tar.zst`, `manifest.txt`가 다시 검증됐고 다음 timer는 12:22:24다. `BAK-RUNTIME-177-01`은 backup 생성이 restore 증명이 아니므로 isolated decrypt/restore, schema+migration-set equality, identity/session/inventory/entitlement/ledger/reward/bank/loan/stock/casino/community/referral/audit reconciliation, RPO/RTO 실측, off-host immutable retention, 실제 failure alert 전달까지 IN PROGRESS다. component release authority는 `{frontendSha,backendSha,imageDigest,migrationSetHash,deploymentId}`이며 v213 frontend `24b85df...`와 backend `75e69e77...`의 차이는 backend 승격 전까지 의도된 상태다.
- **SEO/SEO backend — DIRECT ADOPT:** 2026-09-17까지 최신 Google Search 문서는 infinite-scroll 지침 자체는 변경하지 않았고 crawler/user-triggered-fetcher의 제품 영향·encoding을 분리하며 Indexing API 제출은 spam detection 대상임을 명시한다. 따라서 Indexing API를 일반 UGC/catalog 색인 가속기로 사용하지 않는다. public profile/community/gallery/catalog/search landing은 publication+moderation+quality eligibility 통과 후에만 SEO read-model에 들어가며 durable SSR URL, canonical, title/meta/H1, breadcrumb/internal link, 적격 JSON-LD, image dimension/alt, hreflang, 정확한 200/404/410/301/308, sitemap `lastModified`, plain/gzip/br body parity, CWV를 gate로 둔다. private/account/admin/transaction과 불안정 cursor/facet/query는 noindex/excluded 강제다. Naver robots meta도 같은 index-eligibility authority에서 생성해 Google/Naver drift를 막고 redirect target에도 의도한 index 정책을 반복한다. SEO admin은 crawl/index 상태, canonical conflict, sitemap freshness, crawler-family status/latency, reversible eligibility reason을 제공한다. KPI는 organic impression→CTR→visit→signup→activation→D7/D30→revenue다.
- **보안 — DIRECT ADOPT / OWASP ASVS 5.0:** user-session 밖 backend component는 individual service account, short-lived token 또는 certificate와 least privilege를 사용한다(V13.2.1/V13.2.2). auth/session/payment/reward/bank/stock/casino/admin write는 object/function authorization, server-authoritative value, idempotency/replay, DB constraint/transaction, immutable masked audit, anomaly detection을 요구한다. HIGH negative gate는 cross-user BOLA, privileged-field mass assignment, stale/revoked session replay, 잘못된 issuer/audience/scope service credential, duplicate reward/payment webhook, SSRF destination bypass, DB-role overreach다. 민감 browser response는 `no-store`, logout은 server session/refresh family 폐기와 app-managed browser state 삭제를 요구한다. 예외는 만료·owner·residual risk 기록이 필요하고 payment/admin authorization을 우회할 수 없다.
- **수익성/사업성 — DIRECT ADOPT:** Google Play는 EEA/UK/US updated fee와 remaining market을 분리한다. global rollout이 한국에 적용되기 전 enrolled 15% tier는 연간 첫 USD 1M 15%, 초과 30%, auto-renew subscription은 15%이며 적격 한국 alternative-billing은 otherwise-applicable Play service fee에서 4 percentage points를 낮춘다. 이를 universal constant로 코딩하지 않는다. 모든 SKU/order/refund는 `{market,effectiveDate,installCohort,transactionType,programme,billingPath,feePolicyVersion,gross,platformFee,billingFee,tax,refundReserve}`를 snapshot하며 entitlement authority는 별도다. consumable/non-consumable/subscription/ad-removal catalog는 보수/기준/낙관 unit economics에 attach/repeat/renewal, discount/cannibalization, fraud, infra, support, refund 비용을 포함한다. 미실측 revenue/net revenue/margin/ARPU/ARPDAU/ARPPU/conversion/retention/churn/refund/CAC/LTV/payback은 `HYPOTHESIS/TEST TARGET`이며 contribution margin이 양수이고 D7/D30·공정성·support/fraud guardrail을 지킬 때만 scale, 경계면 iterate, 지속 음수 또는 신뢰/retention 중대 악화면 kill한다.
- **전체 기능 실행계약:** auth/signup/login/OAuth/logout/session/security center, profile, inventory/collection, shop/cart/payment/subscription/ad-removal, season/quest/job/level/reward, business/bank/loan, virtual stock/portfolio/alert/comparison, casino/chance, community/post/comment/report/block, friend/club/invite/referral, notification, search, gallery/upload, public content, App API, admin/audit, backup/restore, analytics/experiment, ads, SEO tooling, Discord/incident 전부 구현상태+코드/문서 근거, 목적/persona/진입/screen+CTA+state matrix, responsive/a11y/i18n, ownership/RBAC, API request/response/error/idempotency/rate-limit, service rule, DB index/constraint/transaction/concurrency, audit/observability/admin/fallback/DR, threat/privacy/abuse, SEO, product+financial KPI, cache/performance SLO, 완료조건, unit/integration/E2E/real-DB/security/regression, exact-SHA Test→main→Production smoke/rollback gate를 유지한다. P0/P1 integrity/security/release가 신규기능보다 우선한다.

### v216 worklog / 수용 순서
Google Search/Crawling + Naver Search Advisor + OWASP ASVS + Google Play 공식조사 → v215 exact-head sync 복구 → exact main/EN/KO 읽기 → Debian service/timer/local API/journal + canonical DNS 재현 → 이슈/보안/SEO/수익성 실행 델타 → 작업 중간 exact-main 재확인 → EN/KO 동기화 → diff/CI/PR. 이 기획 회차에서 runtime/DB 배포는 금지한다.
