...
### 반응형 main 반영 대조; DR/릴리스 권위는 계속 최우선 차단조건

- **정확한 main/구현 증거:** 시작 및 작업 중간 기준은 `7d2809465188406515bf822bf5e1ec8e3a9a47c0`, 반응형 PR #449 병합본이다. 따라서 v201 반응형 변경은 **MERGED**이나 자동으로 Production-proven은 아니다. branch protection은 활성화됐지만 required status checks는 `enforcement_level=off`, required context/check 0개로 **P1 / OPEN**이다. classifier/policy/runtime/security check를 repository-required로 만들고 의도적 실패 check가 merge를 실제 거부하는 증거가 완료조건이다.
- **P0 백업/DR `BAK-RUNTIME-177-01`: IN PROGRESS; 2026-09-18 01:05 KST 재현.** 권위 Debian에서 backend/frontend/backup timer가 active, timer가 enabled다. 00:22 실행은 encrypted archive와 `database.dump`, `photos.tar.zst`, `manifest.txt`를 모두 검증했고 다음 실행은 06:26이다. 이는 로컬 암호화 백업 생성 증거이지 복구 가능성 증거가 아니다. DONE에는 최신 archive isolated restore, decrypt/checksum/schema/migration-set 검증, identity/session·inventory/entitlement·ledger/reward·bank/loan·stocks·casino·community/referral·audit reconciliation, 실측 RPO/RTO, off-host immutable replication/retention, missed-run/verify/replication/restore alert 실제 전달이 필요하다. 불일치는 fail-closed이며 완료 전 Production restore 및 destructive schema/ledger/entitlement rewrite를 금지한다. 롤백은 마지막 정상 archive/key를 보존하고 고장난 자동화만 비활성화한다. last-success age, archive size/checksum, free space, replication lag, restore result, alert delivery를 관측한다.
- **런타임/릴리스 권위:** 01:05 KST backend `/api/version`은 HTTP 200, `Cache-Control: no-store`, deployed application `75e69e77cdc18ef221106a008563151a4c790728`이며 최근 1시간 warning 이상 backend journal은 비어 있다. repository main은 더 최신이므로 deployed identity로 간주하지 않는다. `REL-AUTH-184-01`은 **P0 / IN PROGRESS**이며 CONTROL_PLANE_ONLY ×3, RUNTIME_RELEVANT exact-candidate ×2, MIXED ×1, missing/stale/foreign candidate 음성 케이스가 candidate binding과 무권한 side effect 0을 증명해야 한다. 승격은 exact candidate `{applicationSourceSha,imageDigest,migrationSetHash,deploymentId}` → exact-SHA Test → backend/API/DB/user-flow QA → main → Production → smoke/rollback manifest 순서다.
- **반응형 UX `UI-RESP-202-01`: P1 / MERGED, Production 증거 대기.** #449의 drawer, 5-tab bottom navigation, banking, inventory 좁은 화면 수정은 main에 들어왔다. exact-main/Test에서 public + authenticated banking/inventory 상태를 320/360/390 px로 재검증하고 loading/empty/error/offline/timeout, 200% zoom/reflow, keyboard/focus order, 긴 KO/EN label, 적용 가능한 ≥44 px touch target, document/component 가로 overflow 0을 확인한 뒤 실제기기 Production smoke와 error/CWV 비교가 필요하다. navigation/session/core CTA 접근성이 퇴행하면 이전 attested frontend image로 롤백한다.
- **전체 기능 실행계약:** auth/signup/login/OAuth/logout/session/security center; profile; inventory/collection; shop/cart/payment/subscription/ad-removal; season/quest/job/level/reward; business/bank/loan; virtual stocks/portfolio/alerts/comparison; casino/randomized mechanics; community/posts/comments/report/block; friends/clubs/invite/referral; notifications/search/gallery/upload/public content; App API; admin/audit; backup/restore; analytics/experiments; ads; SEO backend/tooling; Discord; incident operations 모두 화면 entry/CTA/state recovery, responsive/a11y/i18n, ownership/RBAC/BOLA, endpoint schema/error/idempotency/rate-limit, service rule, DB PK/FK/unique/check/index/transaction/concurrency, immutable audit/observability, admin/flag/fallback, privacy/abuse, SEO, event/KPI, performance/cache, unit/integration/E2E/real-DB/security/regression, deploy/rollback을 유지한다. exact 코드/문서/환경 증거 없이는 DONE이 아니다.
- **SEO/SEO 백엔드 — 직접 채택:** Google Search Central 최신 로그의 2026-09-17 infinite-scroll 지침 이전은 내용 변경이 없다. 공개 community/catalog/collection/search infinite scroll은 crawlable pagination URL, server-renderable link, stable ordering, page별 self-canonical/title/H1, 정확한 200/404와 독립 가치 page만 sitemap 포함해야 한다. cursor/facet은 SEO read-model이 영구 landing으로 승격하지 않는 한 API-only 또는 noindex/canonical이다. metadata/canonical generator, robots, split sitemap+`lastModified`, breadcrumb/JSON-LD, hreflang, SSR/ISR, image metadata, permanent redirect map, crawler log, Search Console/Naver ingestion은 백엔드 책임이다. private/account/admin/security/transaction은 `noindex`; KPI는 impression→CTR→organic visit→signup→activation→D7/D30→revenue와 thin/duplicate/CWV guardrail이다.
- **보안 — 직접 채택:** OWASP API Security Project의 최신 API-specific Top 10은 계속 2023이다. BOLA, broken authentication, object-property/function authorization, resource exhaustion, sensitive-business-flow abuse, SSRF, misconfiguration, API inventory, unsafe upstream consumption을 release-block한다. 외부 API는 TLS, 필요한 경우 destination allowlist, schema validation/sanitization, 제한된 redirect/timeout/body/concurrency, circuit breaker/fallback, masked log가 필요하다. cookie mutation은 CSRF 방어, payment/reward/ledger mutation은 server-authoritative price/reward, signed receipt/webhook, replay-safe idempotency, DB least privilege, immutable audit, fraud/multi-account 탐지가 필요하다. HIGH 잔여위험은 owner가 필요하고 명시적 수용 없이는 승격을 막는다.
- **광고/CSP `ADS-CSP-202-01`: P2 / OPEN.** #449 QA의 Gallery Google ad-quality `sodar2.js` CSP rejection은 남아 있다. CSP를 광범위하게 완화하지 않는다. exact main/Test에서 response CSP+console+initiator로 재현하고 구성된 광고 제품과 연결한 뒤 필요한 경우 공식 요구사항의 정확한 origin/directive만 추가한다. 이후 XSS/security-header 음성 테스트와 Gallery E2E를 재실행한다. 사업판정은 request 수가 아니라 provider impression/revenue에서 Gallery abandonment, session loss, D1/D7 악화를 뺀 순효과다.
- **수익성/unit economics — 직접 채택:** Google Play 현재 문서는 universal fee를 두지 않는다. 새 EEA/UK/US 모델의 standard 예시는 auto-renew subscription 10%, 기타 new-install 20%, 기타 existing-install 25%이며 해당 시 5% billing fee가 붙는다. 모든 SKU는 `{market,effectiveDate,installCohort,transactionType,programme,billingPath,tax/refund assumption}`을 resolve한 뒤 gross→net→contribution margin을 계산한다. 카탈로그는 server-authoritative SKU/price, WLD/실결제, consumable/non-consumable/subscription, stock/window/limit/duplicate/account-binding/gifting/refund/restore/cancel/renewal/grant, economy source/sink, P2W 평가를 유지한다. 미실측 ARPU/ARPDAU/ARPPU, attach/repeat, conversion, renewal/churn/refund, CAC/LTV/payback, ad eCPM/fill/CTR/net churn, infra/support/fraud, D1/D7/D30은 `HYPOTHESIS/TEST TARGET`만 허용하고 kill/iterate/scale은 cohort와 공정성/신뢰 guardrail을 요구한다.
- **우선순위/실행 백로그:** P0 isolated restore + off-host immutable replication + alert 증거 → P0 잔여 release-authority matrix → migration-204 real-DB/economy-integrity → HIGH auth/BOLA/CSRF/idempotency/ledger/payment/webhook/upstream-abuse → P1 exact-main responsive Test/Production 증거 + required-check enforcement → Discord Production smoke → P2 Gallery ad-CSP 판정 → core correctness → monetization → SEO/acquisition → retention/accessibility. 이번 기획은 runtime/DB를 변경하지 않는다.

### v202 worklog / 수용 순서
최신 Google Search Central·OWASP·Google Play 공식자료 → exact main/canonical/branch-protection 대조 → 권위 Debian services/version/backup/journal 재현 → 전체 기능/SEO/보안/수익성/UX/QA 델타 → 작업 중간 exact-main 재확인 → EN/KO 동기화 → diff/CI/PR.
