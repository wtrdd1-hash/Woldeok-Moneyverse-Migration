...
- OpenAlex/Crossref 후보 35,429건을 수집하고 DOI 우선/제목 보조 중복 제거 후 **고유 후보 33,341건**으로 탐색 범위를 확장했다. 코퍼스 규모는 탐색 폭이며 전수 원문 수작업 검토 수가 아니다.
- 기본 분산 포트폴리오를 무료 핵심, 맥락형 공개광고, `Moneyverse Plus` 광고제거/편의/표현 가치, 비P2W 코스메틱 직접 entitlement, 명확한 스폰서십으로 채택한다.
- 프리미엄 수요를 만들기 위해 무료 핵심 기능을 의도적으로 악화하지 않는다. 유료 가치는 인위적 불편 제거가 아니라 추가 가치여야 한다.
- 유료 코스메틱/표현 상품은 결제 entitlement로 직접 구매하며 현금으로 WLD를 먼저 사는 중간 구매단계를 두지 않는다.
- 지갑, 송금, 대출, WDX 판단/주문, 카지노, 계정보안, 결제실패, 관리자 action 화면에는 광고를 계속 금지한다.
- 광고 실험은 홀드아웃과 사전 등록된 유지율, 작업완료, 지연, 불만/오클릭, 신뢰 가드레일을 요구한다. 광고 총매출만으로 가드레일을 무시할 수 없다.
- `WLD_CASH_PURCHASE`, `PAID_RANDOM_ITEM`, 유료 카지노 가치/베팅, P2W 경제 우위, 유료 우월 WDX 정보는 계속 BLOCK이다.
- 스토어/결제 단위경제는 시장/채널/시행일/해당 시 설치코호트/거래유형/결제경로/프로그램별로 계산한다. 발표된 미래 수수료 일정을 시행 전에 적용하지 않는다.
- 구독 UX는 국가별 최소요건과 별개로 간단한 온라인 셀프 해지, 명확한 반복결제 조건, 영구적인 동의/가격버전 증거를 유지한다.
- 수익성은 스토어/플랫폼/PSP 수수료, 세금, 환불/차지백, 수익배분, 증분 인프라, 지원/모더레이션, 현지화/콘텐츠, 컴플라이언스/법무 운영비 차감 후 기여이익으로 본다.
- 가격, 전환율, eCPM, churn, CAC, LTV, 매출예측은 Moneyverse 텔레메트리 실측 전까지 `HYPOTHESIS/TEST TARGET`이다.
- 근거: `../findings/MONEYVERSE_MONETIZATION_REVENUE_RESEARCH_REVIEW_v2026.09.25.444.ko.md`; 기획 델타: `deltas/v2026.09.25.444.ko.md`.
