...
echo "=== [1/4] Promoting Test Environment to v406 ==="
sudo ln -sfn /srv/moneyverse-data/releases/test-v406 /srv/moneyverse-data/releases/test-current
...
echo "=== [3/4] Blue-Green Zero-Downtime Promoting Production to v406 ==="
sudo ln -sfn /srv/moneyverse-data/releases/prod-v406 /srv/moneyverse-data/releases/production-current
...
PROD_BANK_HTTP=$(curl -sS -o /dev/null -w "%{http_code}" https://easy-scraping.com/bank)
PROD_SEASONS_HTTP=$(curl -sS -o /dev/null -w "%{http_code}" https://easy-scraping.com/seasons)
...
echo "Production Notifications BFF Status: $PROD_NOTIF_HTTP"
echo "Production Stocks HTTP Status: $PROD_STOCKS_HTTP"
echo "Production Portfolio HTTP Status: $PROD_PORTFOLIO_HTTP"
echo "Production Bank HTTP Status: $PROD_BANK_HTTP"
echo "Production Seasons HTTP Status: $PROD_SEASONS_HTTP"
...
echo "=== Blue-Green Promotion to v406 Completed Successfully! ==="
