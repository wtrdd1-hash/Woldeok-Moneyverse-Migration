...
cleanup() {
  ...
  if [[ "$edge_mode" == 'canary' ]]; then
    if wait_http "http://127.0.0.1:$PRIMARY_BACKEND/health" 3 && wait_http "http://127.0.0.1:$PRIMARY_FRONTEND/api/version" 3; then
...
wait_http "http://127.0.0.1:$CANARY_FRONTEND/api/version" || fail 'canary frontend did not become healthy'
assert_version "http://127.0.0.1:$CANARY_FRONTEND/api/version"
...
wait_http "http://127.0.0.1:$PRIMARY_FRONTEND/api/version" || fail 'primary frontend did not recover'
assert_version "http://127.0.0.1:$PRIMARY_FRONTEND/api/version"
