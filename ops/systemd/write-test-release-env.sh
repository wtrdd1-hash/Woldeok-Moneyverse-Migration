...
SHA="${1:?usage: write-test-release-env.sh SHA OUTPUT_DIR}"
OUTPUT_DIR="${2:?usage: write-test-release-env.sh SHA OUTPUT_DIR}"

fail() { printf "test-release-env: %s\n" "$*" >&2; exit 1; }
...
mv -f "$backend_tmp" "$OUTPUT_DIR/test-backend-release.env"
mv -f "$frontend_tmp" "$OUTPUT_DIR/test-frontend-release.env"
trap - EXIT
printf "test-release-env: OK sha=%s output=%s\n" "$SHA" "$OUTPUT_DIR"
